...
def main() -> None:
    ...
    months = [
        ...
        },
        {
            ...
            "paid": "柳柳", "paid_base": "圣诞轻换装",
            "rank": "凌霜", "rank_base": "全新圣诞主题｜雪夜女王",
        },
    ]
    ...
    def hero_cell(month: dict, kind: str) -> str:
        ...
        if kind == "paid":
            spec = "节日装扮＋偏静态视频 · 较弱BUFF · 无任何技能"
            css = "paid-cell"
        else:
            spec = "全新节日主题＋更高规格视频 · 更强BUFF · 技能可主动使用"
            css = "rank-cell"
        return (
...
</style></head><body><main class="page"><header class="hero"><small>X3 · 节日外显模块养成线</small><h1>英雄皮肤三档排期｜9–12月</h1><p>决策页 v2 · 2026-08-05</p></header><div class="thesis">一句话分档：<em>免费档</em>从稀有/史诗英雄中选现有老皮做自选，用免费参与消化存量；<em>$19.99档</em>选高覆盖英雄换节日装扮，视频偏静态、BUFF较弱且无任何技能，承担付费破冰；<em>排行榜档</em>选高人气传奇英雄做全新主题，用更高规格视频、更强BUFF和可主动使用的皮肤技能建立旗舰竞争价值。</div><section class="shots">__SHOTS__</section><div class="section-title"><h2>9–12月三档内容甘特图</h2><span>横轴=节日月份 · 纵轴=投放档位</span></div>__GANTT__<div class="decision-note"><b>当前取舍：</b>8款免费旧皮拆成4期、每月2选1，四个月不重复；若坚持9月一次8选1，则10–12月必须另补免费候选池。每月三档英雄互不重复；12月$19.99档柳柳与9月同档复用，间隔3个月。</div><details class="secondary"><summary>二级界面｜查看数据依据、免费池与D35前完整资产</summary><div class="secondary-body"><h3>为什么$19.99先用这些英雄</h3><div class="proof-grid">__PROOFS__</div><p>轻付费档优先选择高覆盖英雄，降低玩家因英雄未解锁而无法购买的阻力；柳柳承接9月周年和12月圣诞轻换装。排行榜档必须选择高人气传奇英雄；赛米拉上调为周年旗舰，凌霜上调为12月“雪夜女王”旗舰。英雄解锁数只能证明覆盖规模，最终仍需用英雄出战率、偏好/投票或内容互动数据复核。</p><h3>免费档首批8款</h3><div class="pool-grid">__FREE_POOL__</div><details class="nested"><summary>D35前42位英雄池</summary><div class="hero-grid">__HEROES__</div></details><details class="nested"><summary>现有48款皮肤明细</summary><div class="table-wrap"><table><thead><tr><th>英雄</th><th>英雄品质</th><th>皮肤</th><th>皮肤标签</th><th>获取人数</th><th>获取率</th><th>BUFF</th></tr></thead><tbody>__SKIN_ROWS__</tbody></table></div></details><div class="foot">数据口径：D35前严格按D0–D34；英雄解锁人数与皮肤获取率沿用2026-07-29成熟服专项快照。获取率=曾获得皮肤人数÷对应英雄解锁人数。英雄品质与皮肤品质是两条独立轴。</div></div></details></main></body></html>'''

    paid_proofs = "".join(
        f'<div class="proof"><b>{esc(name)}</b><span>{hero_map[name]["hero_owners"]:,}人解锁 · 传奇英雄</span></div>'
        for name in ["柳柳", "霍普金斯", "弗莱彻"]
    )
