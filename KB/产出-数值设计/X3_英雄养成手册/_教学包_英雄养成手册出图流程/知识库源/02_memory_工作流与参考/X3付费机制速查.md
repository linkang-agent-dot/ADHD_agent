...
### 储蓄罐改「自然日(UTC)限购、0点重置」实现法(2026-06-22异国美酒每日双档C方案落地)
需求:某储蓄罐每档每自然日限购1次、UTC 0点统一重置(非滚动CD)。`ColdTime` 是"从购买时刻滚动N时长"，做不了自然日→需代码。改法(2文件,0配置改动,储蓄罐Pack保持ColdTime>0仅作"购买写时间戳"标志):
- **服务端** `GiftMeta.cs` CheckCanBuyGift 的 `if(cfg.ColdTime>0)` 块:加分支——目标罐按 `GameTime.GetUtcDateTime(lastBuyingTime).Date == GameTime.GetUtcDateTime(now).Date` 则拦(同UTC日买过=今日已砸);其余维持 `now-lastBuyingTime<ColdTime`。lastBuyingTime 取 groupBuyingTimes[Group] 或 buyingTimes[ID](ChangeGiftBuyInfo 仅ColdTime>0时写,见GiftData.cs:360)。
- **客户端** `UI/ItemObtain/UIPiggyBankContent.cs` 2处:①PrefixData 选档②RefreshContent 倒计时endTime——目标罐按UTC同日判断/倒计到次日UTC0点(`now+(nowUtc.Date.AddDays(1)-nowUtc).TotalMilliseconds`),其余走原逻辑。
- ⚠️**必须按"具体哪个罐"精确圈定,别用 `PackType==PACK_TYPE_PIGGY`**(那会误伤所有储蓄罐:金币小中大72h/金海等→全变每日1次=回归)。圈定法(已提常量,2026-06-22重构):常量 `GiftConst.DAILY_RESET_PIGGY_RES_ID=7002`(CSShared共享,服务端+客户端共用);服务端 `extraInfo.piggyExtraInfo.piggyCfgId → CPiggyBank.I(id).ResourceID==该常量`;客户端用计算属性 `UIPiggyBankContent.IsDailyResetPiggy => ItemUtils.GetItemCfgID(Data.itemID)==该常量`(单一来源Data.itemID,选档+倒计时共用)。sub-agent审过无回归(全表仅行46/51是7002)。TODO:多个资源需此行为时改集合/PiggyBank配置标志位
- 关键常量:**X3日界=UTC 0点**(`BasicMeta.DailyUpdate.cs` 按 `(nowDate-lastFireDate).Days` 算跨天;`OnDayUpdate(dayPassed,now)` 是各Meta的0点回调);**GameTime.Time=毫秒**(`GetUtcDateTime()`=当前/`(t)`=指定时间戳转UTC DateTime,客户端服务端都有)。
