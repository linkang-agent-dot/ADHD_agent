...
### 4.4 开箱活动(ActvType=15/UIActvCrafting) prefab 结构图（2026-07-21）
- prefab=`Prefab/Activity/UIActvCrafting.prefab`，**嵌套 prefab 风格**（370 个 PrefabInstance，与马戏扭蛋机的全拍平相反；双击子节点会跳子 prefab）。绑定路径全表=`Auto_UIActvCrafting.cs:34-83`。
- `Root/Animation` 四大区：**Top**（txt_title/txt_desc/Time + Info规则/Prob概率 + UIBtnGift链式礼包 + DailyGift + UIBtnProperty + ActvRank×2跨服本服）｜**Middle**（Temp消耗展示/txt_cost + FlyRoot开箱动画(FlyItem/Box1/Box2+AnimRoot) + Log跑马灯(MarqueeText)+BtnLogList记录 + Jump来源跳转）｜**OtherReward**（Slider进度+BoxGroup里程碑宝箱,模板类 UIActvCraftingBoxSlotItem）｜**Bottom**（BtnCraft单开/BtnOpenAll全开）；外加 Root/BG+BottomBg+Role立绘、BtnRandomReward、Sp_Mask。
- 节日开箱全是此骨架换皮：元旦101513/情人节101514/春节101515/世界杯101516（ActvOnline ActvType=15，历史 const 名 CRAFTING）。

### 4.5 活动礼包入口=通用挂件 UIBtnGift（2026-07-21 开箱案查明）
- **⚠️先查再挂：同一「链式礼包入口」逻辑有两套壳，接一套就够**——①`UIBtnGift` 挂件（prefab=`Prefab/Button/UIBtnGift.prefab`，`WidgetContainer.AddSingle<UIBtnGift>(节点)`+`ShowGift(activityId)`，零子节点绑定，开箱 UIActvCrafting 挂点=`Root/Animation/Top/UIBtnGift`）；②`UIHelper.SetActivityBaseInfo(goGiftEntry: 节点)` 通用参数（内部 ShowActivityGiftEntry，马戏扭蛋机走这条）。两者同读 `ActvOnline.ChainPackID`（=0 永不显示）、同样自动显隐/红点/OpenChainPack 弹窗。窗口若已调 SetActivityBaseInfo，直接传 goGiftEntry 最省；孤立场景（HUD/非活动窗）才用挂件。同顶栏另有 `DailyGift` 挂件（每日礼包，`Show(activityId)`）。
- 顺带：开箱抽奖活动 ActvType=15，const 历史名 TRIGGER_TYPE_CRAFTING「制作活动」，客户端类=UIActvCrafting——搜"Box/Chest"找不到它。
...
## 控件模式：拉伸锚定节点在 prefab 编辑模式下"错位"是正常的（2026-07-17 马戏扭蛋机案）
- **现象**：prefab 里某块内容（如顶部标题栏 Top：标题/倒计时/描述）在编辑视图里跑到画面中下方压住别的元素，运行时位置却正确。
- **原因**：该节点是**上下拉伸锚定**（anchorY 0→1）+ 负 sizeDelta（如 -1317）+ 固定像素偏移——它的实际矩形 = 父节点当时的高度减去固定值。prefab 编辑模式下根节点没有被真实 UIRoot 撑满（隔离模式无 UI 环境画布 / Game 视图分辨率≠设计分辨率），拉伸节点被压扁折叠 → 子元素看似掉到中间；固定尺寸+中心/角锚定的兄弟节点不受影响，所以只有这一块"错位"。运行时根被全屏画布(1640×2560)撑开，偏移量按设计展开 → 位置正确。
- **判定法**：选中该节点看 Rect Transform——若显示 Top/Bottom/Left/Right（拉伸模式）而非 PosX/PosY，就是这个模式，编辑器里的视觉位置不可信，以 Game 视图真实分辨率为准。
- **配套工具**：`C:\ADHD_agent\skills\unity-prefab-tools\prefab_rect_tree.py`——不开 Unity 直接打印 prefab 节点锚定参数/父链/按 languageKey 定位节点（--node / --chain / --key 三模式）。
- **让 prefab 模式显示和真机统一（零改动，2026-07-21 实测有效）**：prefab 隔离画布尺寸跟随 **Game 视图分辨率**——Game 视图分辨率下拉 `+` 新建 **Type=Fixed Resolution** 的 1080×1920，选中后**停掉 Play、重开 prefab** 即正确展开。⚠️两个坑：①**Aspect 比例条目（`1080:1920` 冒号）不管用**——只锁比例、实际像素跟 Game 窗口物理大小走，小窗口高度仍<1317 照样折叠；必须选带 `x` 的固定分辨率条目（`1080x1920`）。②改完分辨率必须退出再重开 prefab（隔离画布在打开时取尺寸）。横屏/小 Game 视图（高<1317）会让 Top 这类负 sizeDelta 节点翻折到画面中间。对照实证：UIActvLaborGacha（克隆源）Top 参数与马戏版完全一致，整个活动界面家族在 prefab 模式下同样表现。工程已配 PrefabUIEnvironment=UIScene.unity 但该场景只有 Camera+EventSystem 无 Canvas，别指望它。
- **别去"修"它**：手动把它拖回"看起来对"的位置反而会把运行时位置改坏。
- **改 UI 元素颜色的正确姿势**：Image 的 Color 是**乘法染色**——白图随便上色，彩图只会越乘越暗/发脏，变不出亮色。要换色=先换 **`NewSprite/Common/WhiteImg/`** 目录的白底图（该目录专为染色设计，如 `Common_frame_QuestionMark_` 白问号、`Common_WhiteImg_TickMark` 白勾；⚠️**没有白图感叹号**，感叹号只有彩色版如 `Spirits/Alliance/icon_notice`）再用 Color 上任意色。马戏案实证：棕金问号钮换白图+染 355B82/986D2E 与标题成套。
- **姊妹现象——节点拖不动**：子节点被父节点的 LayoutGroup/ContentSizeFitter 接管（Inspector 灰字 "driven by ..."），拖了会被吹回。挪整块=拖父节点；调块内间距=改 LayoutGroup 的 Padding/Spacing/Alignment；要脱管=子节点加 LayoutElement 勾 Ignore Layout（少用）。实例：马戏扭蛋机 AutoTrain（自动抽条，HLayoutGroup+CSF）下的 Toggle。
- **通用道具格模板结构（马戏扭蛋机实证，X3 活动界面通用）**：格子=`ItemTemplate/ItemMid/Scale/{Icon(图标本体·代码按道具ID填图)、frameList/Frame1-5(品质框)、HeroIconMask+heroFrameList(英雄专用)、fx(可领取发光)}`+`ItemMid/{Count,Lv}`。改显示哪张图=改配置不改 prefab（代码覆盖 sprite 不覆盖 rect）；调大小动 `Scale` 整体缩。**调格子大小的正确层级（马戏案三轮踩坑定论）**：只动 **ItemMid 的 localScale**（图标+品质框+**数量标 Count+等级标 Lv** 一起等比缩，Count/Lv 是 ItemMid 直属子节点、但不在 Scale 下）；**别动 Scale 节点**——它只套图标+框，动它会让图标单独变大而 Count/Lv 不跟→数字偏小、脱离右下角（本案 Scale 0.8→1.0 就是这个洋相，已退回 0.8）。格子密度另调 GridLayout 的 CellSize。三个约束联动：①CellSize×行数 ≤ 面板可视高（超了第二行探出框，本案 cell120×2=240 超蓝面板~210）②图标视觉尺寸(202×0.8×ItemMid) ≤ CellSize 否则邻格重叠 ③列数用 Constraint=1(FixedColumnCount)+ConstraintCount 锁死，否则按宽度自动塞列（cell 缩小会从 3 列变 4 列）。马戏终值=cell105/ItemMid0.64/Scale0.8/3列居中/列表区488×210。
**编辑器预览多格排版**：Ctrl+D 复制 ItemTemplate 拖进 `Viewport/Content`（挂 GridLayoutGroup，Cell/Spacing/Padding 全在它身上），看完**必删复制品再保存**（运行时按真数据克隆，留着出双份）。**调运行时填充的 Icon 大小**：先给 Icon 塞一张真实道具图当占位（Source Image ⊙ 搜 `icon_global_*`，X3 道具图标库在 `Res/UI/Spirits/ItemIcons/`）+ 眼睛藏掉英雄专用层（HeroIconMask/heroFrameList），按真实观感调；占位图可保存——代码只覆盖 sprite 不覆盖 rect。模板里的 Lv99 头像/555 数量都是占位假数据，别据此判断资产归属（判 X2/X3=查引用面）。**外部改用户在编辑的 prefab（agent 直改磁盘文件）三件套**：①改前 `git diff --stat` 对账，区分用户已保存改动和自己的；②改后比对 HEAD↔当前的 `m_Name` Counter 差集——能抓出用户在 Unity 里**误删的节点**（马戏案实证：用户清视野误删 Cont/BtnBlue 积分入口+Top/GiftEntry 礼包入口，都是代码注册了点击事件的活按钮，靠这招发现）；**误删恢复=子树级 YAML 手术**（马戏案全流程实证）：从 git HEAD 收集目标 transform 子树全部 docs（GO+全组件）→ 追加回当前文件（fileID 引用制，文档顺序无关紧要）→ 父 RT 的 m_Children 按原 sibling index 插回；⚠️fileID 冲突≠一定是坏事——先查冲突 doc 的 RT `m_Father` 是否漂移：用户拖出子节点再删父会留下**孤儿节点**（本案 BtnBlue/icon 被拖挂到 Cont），孤儿=整块回写 HEAD 版并从现父 children 摘除。收尾验证三件：对象总数=HEAD、m_Name 差集空、自己此前的改动仍在；范例实现=`skills\unity-prefab-tools\examples\circus_prefab_batch.py`（三批打包：换图+关层+子树接回）；脚本断言写**目标态**（"最终6层关"）别写动作数（"本次关6层"）——重跑/半程状态下动作数必误报；③**铁律：必须等用户「Ctrl+S 存完 + 退出 prefab 编辑模式」之后才动文件**——只提醒"回去选 Reload"没用（马戏案血案实证：用户 Unity 全程没重载过磁盘，prefab stage 一直拿内存旧态覆盖保存，agent 的换图+子树手术两批改动全被 19:22 一次保存清零）。正确时序=用户存好退出→agent 改文件→用户重新双击打开（现读磁盘）。改完立即回查文件（对象数/关键 guid 计数），发现被盖就别再改，先把用户拉出编辑态。
- **姊妹现象——编辑器里飘着孤立特效/白块**：多半是 ItemTemplate（格子模板）里的功能特效（如 Fx_UI_JiangLiKeLingQu_Glow=奖励可领取发光），代码按路径绑定（如 `ItemMid/Scale/fx`）、运行时克隆到每个格子并控制显隐——**别删别挪**；编辑时嫌碍眼用 Hierarchy 行首的**眼睛图标**（快捷键 H；搜索框搜 `Fx_`/`Particle` 全选批量藏），⚠️两个限制：①别用 Inspector 的 active 勾选框关（会存进 prefab）；②眼睛/H **只影响 Scene 视图**——Game 视图和 Play 运行画面不受它管。另：Scene 里"白色小方块"若不是粒子，先怀疑没指 Sprite 的 Image（渲染成纯白块），可用 prefab_rect_tree 同目录思路搜 `m_Sprite: {fileID: 0}` 反查。
- 顺带：克隆界面的 Top 栏 languageKey 常是源界面遗留（如 Text_HeroUpRoute_Title 编辑器预览显示"天下第一武道挑戰"），运行时代码 mTFWTextTopTitle 等会填真文案，占位文本不用管、节点名别改（Auto_ 按路径绑定）。

- **YAML 手术给现有节点外包一层（如 ScrollRect 包 LayoutGroup 条）必查 Auto_ 全路径绑定（大富翁存钱罐奖励条案实证）**：`Auto_<UI>.cs` 的 `FindByFullPath/GetComponent<T>("A/B/C节点全路径")` 是按**层级路径**绑定的——reparent 插入一层（`Detailed/BoxGroup` → `Detailed/BoxScroll/BoxGroup`）后这些绑定静默返 null → OnLoad 里 `null.AddRefreshBarEvent(...)` NRE → **整个 OnLoad 中断，界面半初始化**（症状分散且看似无关：锁图标不刷新、动态列表一个不生成、事件没注册——其实是同一个 NRE 根因）。修=手术同时把 Auto_ 文件里所有经过该节点的路径补上新层级（生成文件手改与重跑生成器等价）。排查口诀：改完 prefab 层级后 `grep 旧路径 Auto_*.cs`；运行期"多个不相干功能同时失灵"先怀疑 OnLoad NRE。
- **横向滑动条改造·开工工序（大富翁奖励条案 6 轮返工换来的,先于下面配方读）**：①**找参照要"同用途"不是"形似"**——同工程里找一个和你场景一样(静态条 vs 循环列表)且在线上真实工作的实现整块参照;要抄的每个组件先 guid 反查类名读源码确认用途(本案血债:整块抄了 HeroChampionReward 的滚动块,实为 LoopScrollRect 虚拟化列表,裸配完全拖不动,返工 3 轮) ②**事件类功能先画完事件链再动手**——UGUI 拖拽链=raycast命中最上层Graphic→向上找第一个IDragHandler(会被格子 Listener 截胡)→转发只认显式SRFilter;三个相关类(TFWScrollRect/ScrollRectFilter/EventTriggerListener)源码合计20分钟读完,比边猜边试省一晚上 ③改不动时**换思路找最小改动点**(张力终解=关格子图形raycast让事件穿透,根本不碰转发链)。
- **横向滑动条改造配方（挤压→可滑）**：均摊式 HorizontalLayoutGroup（ChildForceExpandWidth=1+子项 0 宽）项多必挤压。改滑动=①子项模板给真实宽(如120) ②HLG 改 ChildAlignment=MiddleLeft+ForceExpandWidth=0 ③外包 BoxScroll 节点(**普通滚动用 TFWScrollRect guid `93998fea94671024680976c492306d00`**,ScrollRect 基类字段+`handleDrag: 1`+`scrollable: 1`;+RectMask2D `3312d7739989d2b4e91e6319e9a96d76`+隐形 raycast Image 当拖拽热区,viewport=自身)。⚠️**别抄 `0f4296fa`=LoopScrollRect(循环列表虚拟化,要 totalCount/数据源初始化,裸配=完全拖不动)**——参照 prefab(HeroChampionReward等)带 m_Horizontal 的多是 loop 列表,盲抄必翻车;`ca6dfd03`=ScrollRectFilter(点击/滑动区分器,配 ClickHelper 的 enableScrollRectFilter 用) ④content 加 ContentSizeFitter(`3245ec927659c4140ac4f8d17403cc18`,HorizontalFit=Preferred) ⑤别忘上一条的 Auto_ 路径同步 ⑥**头号暗坑=拖拽事件被格子吞掉（"完全拖不动"的真因,大富翁奖励条案代码铁证）**：`EventTriggerListener`(ClickHelper,guid 1da2a900) 实现了 IBeginDrag/IDrag 接口——EventSystem 把拖拽发给最近的 handler(=格子)就停,它的转发**只认显式 `SRFilter` 字段**(`if (SRFilter != null) SRFilter.FilterEvent(...)`,无 GetComponentInParent 兜底);格子的监听器可能是模板 prefab 自带(序列化 SRFilter={fileID:0})或运行时挂载(天生 null),两种都中招→事件到格子为止,ScrollRect 永远收不到。**修=克隆/绑定完后代码遍历 content `GetComponentsInChildren<EventTriggerListener>(true)` 把 SRFilter 统一指到滚动节点上的 ScrollRectFilter**(幂等,每次刷新跑);prefab 手术改不了运行时挂载的组件。症状鉴别:完全没反应=事件层(本条)/跟手但回弹=content尺寸/单向=方向或初始位置。**⑦终局定案(张力 3cca315 大富翁滑动正修,两方案对比)**:让滚动区能拖有两条路——a)转发链(给每个格子 Listener 接 SRFilter,运行时遍历接线)b)**穿透方案(张力选型,更干净)=把格子内会挡事件的图形 raycastTarget 关掉(Bg/Receive 等),指针直接命中滚动节点热区,ScrollRect 原生收拖拽,整条转发链不再依赖**;代价=格子自身点击失效(有格子点击需求时只能走 a)。**⑧ChildGroupController 深坑**:它把 gridRoot 容器下**所有子节点**按索引当"项"处理(改名/隐藏),容器里混任何非项节点(Slider/特效)都会被它糟蹋——列表容器要么纯净只放项,要么弃用它改 `WidgetContainer.AddList<T>(模板,容器)` (UIWidgetList) 手动管理;克隆序=**先一次性 Add 够再逐个 BindData**(边绑边克隆会把已绑定的运行时状态深拷贝进新项);UIWidget 子项 BindData 首行 Show()(生命周期)。

## 控件模式：分档积分轨道的"段内进度"填充（2026-07-28 马戏扭蛋机积分条案）
- **症状**：横向档位轨道（一排奖励格 + 门槛数字 + 底部条）的进度条**只在跨档时整段跳一格**，档内加分条不动（总积分数字却是实时的）。
- **根因不是刷新链路，是"段内比例填充压根没实现"**。先排除刷新：阶段积分活动（ActvType=7 单阶段，读 `stageItems[stage].progress`）的加分走 `ActivityMeta.OnUpdateStageSubActivityProgressNtf`（ActivityMeta.cs:2167）→ 末尾 fire `OnRefreshSubActivityUI` → 界面监听它就会每次刷新。**鉴别口诀：数字实时、条不动 = 视觉层没做插值；数字也不动 = 才去查事件监听**（顶层 `progressScore` 走的是另一条 `OnUpdateActivityProgressScoreNtf`:2290，与 stage 分数是两套字段，别混）。
- **X3 标准段内口径（照抄源 = `UIActvBattlePass.BattlePassItem.cs:89-116`）**：`ratio = clamp01((当前分 − 上档门槛) / (本档门槛 − 上档门槛))`。一档一节点的轨道里，把上一档门槛 `prevAim` 沿列表往下传（首档传 0），列表须先按门槛升序排。
- 🪤**X2 搬来的 BlackSlider 模板双层填充语义**：`Img2`(anchor 全拉伸)=**完成段整段亮条**；`Img1`=**进行中段的部分填充**。⚠️**两张都是 Sliced(m_Type=1) 不是 Filled，`fillAmount` 对它们无效**——想当然写 `fillAmount = ratio` 不会报错但毫无效果。
- **查 Image 类型的工具**：`python C:\ADHD_agent\skills\unity-prefab-tools\prefab_rect_tree.py <prefab> --img <父节点名>` 一条命令打出子节点的 Image type/fillMethod/sprite guid + 真实渲染顺序（后者盖前者），不用开 Unity。
