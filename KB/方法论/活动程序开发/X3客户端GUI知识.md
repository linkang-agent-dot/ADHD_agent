...
- **通用道具格模板结构（马戏扭蛋机实证，X3 活动界面通用）**：格子=`ItemTemplate/ItemMid/Scale/{Icon(图标本体·代码按道具ID填图)、frameList/Frame1-5(品质框)、HeroIconMask+heroFrameList(英雄专用)、fx(可领取发光)}`+`ItemMid/{Count,Lv}`。改显示哪张图=改配置不改 prefab（代码覆盖 sprite 不覆盖 rect）；调大小动 `Scale` 整体缩。**编辑器预览多格排版**：Ctrl+D 复制 ItemTemplate 拖进 `Viewport/Content`（挂 GridLayoutGroup，Cell/Spacing/Padding 全在它身上），看完**必删复制品再保存**（运行时按真数据克隆，留着出双份）。**调运行时填充的 Icon 大小**：先给 Icon 塞一张真实道具图当占位（Source Image ⊙ 搜 `icon_global_*`，X3 道具图标库在 `Res/UI/Spirits/ItemIcons/`）+ 眼睛藏掉英雄专用层（HeroIconMask/heroFrameList），按真实观感调；占位图可保存——代码只覆盖 sprite 不覆盖 rect。模板里的 Lv99 头像/555 数量都是占位假数据，别据此判断资产归属（判 X2/X3=查引用面）。**外部改用户在编辑的 prefab（agent 直改磁盘文件）三件套**：①改前 `git diff --stat` 对账，区分用户已保存改动和自己的；②改后比对 HEAD↔当前的 `m_Name` Counter 差集——能抓出用户在 Unity 里**误删的节点**（马戏案实证：用户清视野误删 Cont/BtnBlue 积分入口+Top/GiftEntry 礼包入口，都是代码注册了点击事件的活按钮，靠这招发现）；③告知用户回 Unity 弹「外部修改」选 Reload、且改前确认用户已 Ctrl+S（未保存时用户后保存会覆盖 agent 的磁盘改动）。
