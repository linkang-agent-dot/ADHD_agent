...
PROMPT = (
 ...
 "Phase 7 (from 7s to the end at 14s, a swaying standing idle - THE SWAY IS MANDATORY AND CLEARLY VISIBLE): she sways her body slowly and continuously from side to side, like swaying to slow music: weight onto the left hip... then across to the right hip... and back - one full sway cycle about every two seconds, roughly three full cycles before the end; the sway starts in her hips and flows visibly up through her waist into her chest and shoulders, so her upper body traces a gentle figure-eight; her chest rises and falls with deep, clearly visible breathing layered on top of the sway; her long hair and the pom-pom tinsel swing with the sway, lagging a beat behind; an occasional slow blink; she is NEVER frozen, NEVER standing perfectly still - the body keeps flowing the entire time; her cool, composed, self-assured expression stays unchanged, lips closed; in the final second the sway eases back to center and the clip ends EXACTLY on the final reference frame pose. "
...
for i, u in enumerate(flat(res), 1):
    ...
    dst = os.path.join(OUT, f'intro_full_v11_{i}{ext}')
...
for p in saved[:1]:
    ...
    for tag, idx in (('first', 0), ('mid', n // 2), ('last', n - 1)):
        ...
        if ok: cv2.imencode('.png', fr)[1].tofile(os.path.join(OUT, f'_full11_{tag}.png'))  # cv2.imwrite写不了中文路径
    cap.release()
