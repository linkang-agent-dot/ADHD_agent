...
PROMPT = (
 ...
 "Phase 7 (final ~7 seconds, a long calm idle): she holds this exact pose in a quiet standing idle — only soft natural breathing so the chest gently rises and falls, hair tips and pom-pom tinsel drifting faintly as in a light breeze, a very slight weight shift and gentle slow head sway, an occasional slow blink — her cool, composed, self-assured expression stays unchanged the whole time, and the clip ends EXACTLY on the final reference frame pose. "
...
for i, u in enumerate(flat(res), 1):
    ...
    dst = os.path.join(OUT, f'intro_full_v7_{i}{ext}')
...
for p in saved[:1]:
    ...
    for tag, idx in (('first', 0), ('mid', n // 2), ('last', n - 1)):
        ...
        if ok: cv2.imencode('.png', fr)[1].tofile(os.path.join(OUT, f'_full7_{tag}.png'))  # cv2.imwrite写不了中文路径
    cap.release()
