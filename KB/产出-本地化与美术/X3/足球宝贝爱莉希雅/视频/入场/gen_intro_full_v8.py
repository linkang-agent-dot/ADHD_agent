...
PROMPT = (
 ...
 "Locked camera, plain white background, 15 seconds, one natural whole-body motion line with smooth transitions between phases — never abrupt. "
 ...
 "Phase 2 (~2.3s): an in-place cheer dance — she swings the gold pom-poms left and right with rhythm in her hips — the swings stay clearly below the top edge of the frame and inside the side edges — then circles both wrists in front of her chest so the tinsel flares, one small bouncy hop; all pom-pom swings stay at shoulder height or below, arms never fully extended overhead or sideways. Motion is big but her expression stays understated, a faint knowing smile. "
...
params = {'prompt': PROMPT, 'model': 'seadance', 'ref_types': '首帧图像,尾帧图像', 'duration': 15, 'aspect_ratio': '9:16'}
...
for i, u in enumerate(flat(res), 1):
    ...
    dst = os.path.join(OUT, f'intro_full_v8_{i}{ext}')
...
for p in saved[:1]:
    ...
    for tag, idx in (('first', 0), ('mid', n // 2), ('last', n - 1)):
        ...
        if ok: cv2.imencode('.png', fr)[1].tofile(os.path.join(OUT, f'_full8_{tag}.png'))  # cv2.imwrite写不了中文路径
    cap.release()
