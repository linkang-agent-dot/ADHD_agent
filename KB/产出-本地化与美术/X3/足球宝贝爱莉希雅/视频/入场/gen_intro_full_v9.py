...
PROMPT = (
 ...
 "Phase 2 (~2.0s, brisk): an in-place cheer dance — she swings the gold pom-poms left and right with rhythm in her hips — the swings stay clearly below the top edge of the frame and inside the side edges — then circles both wrists in front of her chest so the tinsel flares, one small bouncy hop; all pom-pom swings stay at shoulder height or below, arms never fully extended overhead or sideways. Motion is big but her expression stays understated, a faint knowing smile. "
 ...
 "Phase 4 (~2.0s, snappy): she rolls the ball back and flicks it UP with her instep; the ball rises in a slow clear arc; she bounces it once on her right knee, once more on her instep, body bouncing elastically, eyes tracking the ball; the ball moves in slow believable arcs, never disappearing or duplicating. "
 "Phase 5 (~0.6s): the ball pops up to chest height and she catches it smoothly between her LEFT forearm and her side, tucking it under her left arm, pom-pom still in hand. "
 ...
 "Phase 7 (from 7s to the end at 14s, a lively sensual standing idle): she stays in place but the idle is alive with a gentle rhythmic SWAY — her weight shifts slowly from one hip to the other, the sway flowing up through her waist, chest and shoulders in one smooth connected wave, head following gently; her chest rises and falls DISTINCTLY with deep, alluring breathing — the breathing motion of her chest is the visual centerpiece of the idle, clearly noticeable and rhythmic, never frozen; her long hair and the pom-pom tinsel swing softly with the body sway, lagging a beat behind; an occasional slow blink; her cool, composed, self-assured expression stays unchanged, lips closed; and the clip ends EXACTLY on the final reference frame pose. "
...
for i, u in enumerate(flat(res), 1):
    ...
    dst = os.path.join(OUT, f'intro_full_v9_{i}{ext}')
...
for p in saved[:1]:
    ...
    for tag, idx in (('first', 0), ('mid', n // 2), ('last', n - 1)):
        ...
        if ok: cv2.imencode('.png', fr)[1].tofile(os.path.join(OUT, f'_full9_{tag}.png'))  # cv2.imwrite写不了中文路径
    cap.release()
