# -*- coding: utf-8 -*-
"""入场片完整版直出(10s单条,无拼接缝): 入场→舞→颠球→夹球→落定。
fflf: 首帧=intro_plate(右缘迈步+球在地) 尾帧=_loop_first(高冷循环首帧)。表情弧线:克制浅笑→专注→渐变高冷。"""
...
PROMPT = (
 ...
 "Phase 1 (~1.5s): she springs into the scene from the right edge with two light bouncy cheer steps, arriving at center; a soccer ball lies still on the ground at center-front. Her expression: a subtle, composed half-smile, lips closed, restrained and confident. "
 "Phase 2 (~3s): an in-place cheer dance — she swings the gold pom-poms high left and right with rhythm in her hips, circles both wrists overhead so the tinsel flares, one small bouncy hop. Motion is big but her expression stays understated, a faint knowing smile. "
 "Phase 3 (~1s): the dance settles smoothly; her gaze drops to the ball, quietly focused; she places her right foot on top of the ball. "
 "Phase 4 (~2.5s): she rolls the ball back and flicks it UP with her instep; the ball rises in a slow clear arc; she bounces it once on her right knee, once more on her instep, body bouncing elastically, eyes tracking the ball; the ball moves in slow believable arcs, never disappearing or duplicating. "
 "Phase 5 (~1s): the ball pops up to chest height and she catches it smoothly between her LEFT forearm and her side, tucking it under her left arm, pom-pom still in hand. "
 "Phase 6 (~1s): she settles — weight sinks calmly, posture straightens to face the camera, and she eases EXACTLY into the final reference frame pose; her faint smile fades CONTINUOUSLY over this final second into the calm, cool, composed gaze of the final reference — chin level, eyes steady, self-assured and aloof; the facial change is gradual, never a switch. "
...
for i, u in enumerate(flat(res), 1):
    ...
    dst = os.path.join(OUT, f'intro_full_v3_{i}{ext}')
...
for p in saved[:1]:
    ...
    for tag, idx in (('first', 0), ('mid', n // 2), ('last', n - 1)):
        ...
        if ok: cv2.imencode('.png', fr)[1].tofile(os.path.join(OUT, f'_full3_{tag}.png'))  # cv2.imwrite写不了中文路径
    cap.release()
