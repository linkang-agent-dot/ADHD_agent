...
LOG = open(r'C:\ADHD_agent\_tmp_gantt_write_log.txt', 'w', encoding='utf-8')
...
def gws_call(subcommand, params=None, body=None):
    ...
    if not stdout.strip():
        stderr = result.stdout.decode('utf-8', errors='replace')
        log(f"  WARN empty stdout for {subcommand}")
        return {}
    try:
        return json.loads(stdout)
    except Exception as e:
        log(f"  JSON parse error: {e} | stdout={stdout[:100]}")
        return {}

def write_values(sheet_name, values):
    """Write values using the +append helper approach: use values.update via path params"""
    params = {
        'spreadsheetId': SPREADSHEET_ID,
        'range': f"'{sheet_name}'!A1",
        'valueInputOption': 'USER_ENTERED'
    }
    body = {'values': values, 'range': f"'{sheet_name}'!A1", 'majorDimension': 'ROWS'}
    result = gws_call('sheets spreadsheets values update', params=params, body=body)
    updated = result.get('updatedRows', result.get('updatedCells', '?'))
    log(f"  write '{sheet_name}': updatedRows={updated}")
    return result
...
gantt_sheets = {
    ...
    '6月拓荒节-甘特': {
        'rows': [
            ...
            ['[风险警告]', 'linkang 独揽以上全部19模块，建议至少分出3-4个', '', '', '', '', '', '', '', '', '', '', '高优先级风险'],
            [],
            ['Y=活跃  空白=未启动  ?=待确认'],
        ]
    },
    '7月烟火庆典-甘特': {
        'rows': [
            ...
            ['行军特效投放', '挖孔小游戏 [7月新增]', '搬运', 'liusiyi', 'S', '','','','','','Y','Y','Y','Y','Y', '仅下半 | S级! 重点资源保障'],
            ['[亮点]', '挖孔小游戏是全部模块中唯一S级，专攻大R/超R', '', 'S', '', '', '', '', '', '', '', '', '', '', '', ''],
            [],
            ['Y=活跃  空白=未启动  ?=待确认'],
        ]
    }
...
for sheet_name, data in gantt_sheets.items():
    log(f"\nWriting: {sheet_name}")
    result = write_values(sheet_name, data['rows'])
