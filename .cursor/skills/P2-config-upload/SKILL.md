...
GSheetDownloader 对每张表**只读该 Spreadsheet 的第一个（最左）页签**。要导某张表的**非首个页签**（临时/审核 tab、或多页签表如 2135 的非首页签）时，必须：

1. **导前先记录原始页签顺序**——用 gsheet API 拉一次该 Spreadsheet 的 `sheets[].title / index` 存下来，全程对页签顺序保持清晰。
2. **把目标页签置顶到第 1 位**（或 `duplicateSheet` 复制一份置顶，作为「备份表」用于导出）。
3. 按下方常规流程跑 GSheetDownloader 导出。
4. **导完恢复原状 —— 仅当本次要导的页签全部导完后才执行**（中途还有其它页签要导就先别还原，攒到最后一次性恢复）：
   - ① **还原页签顺序**到第 1 步记录的原始顺序；
   - ② **删除**为导出临时创建的**备份表**。

> 对应已知现象：`[WARN] not found anywhere` / 策划说加了行但本地无 diff —— 多半是数据在非首页签，Downloader 读不到（见文末「行筛选模式」排查）。
