...
> ⚠️ **模式按「意图」选，不只看输入格式**：
> - **功能验证 / bugfix / 只改了某几行**（如修某活动文案、某 BP 排行榜、某礼包）→ **必须用行筛选模式**：只 merge 本次改动的行 ID，其余行保持 HEAD 不动。**即使你知道表号也不要走全表模式**——全表模式会把 GSheet 上**他人未验证的改动**一起拉进 commit/部署，污染你的 bugfix 提交并可能上线别人没测过的东西。
> - **整表同步 / 批量刷表**（明确就是要让本地 tsv 追上 GSheet 全量）→ 才用全表模式。
> - 拿不准就问用户「只导你改的那几行，还是整表同步？」
...
| 导非首页签后不还原页签顺序 / 不删备份表 | 导表只导**第一个页签**，导非首页签需置顶；导完不恢复会污染表结构（见下方「核心机制」） |

---

## ⛔ 核心机制：导表只导 GSheet「第一个页签」

导表对每张表**只读 Spreadsheet 的第一个（最左）页签**。要导某张表的**非首个页签**时，必须：

1. **导前先记录原始页签顺序**——拉一次该 Spreadsheet 的 `sheets[].title / index` 存下来，全程对页签顺序保持清晰。
2. **把目标页签置顶到第 1 位**（或 `duplicateSheet` 复制一份置顶，作为「备份表」用于导出）。
3. 按下方流程下载/转换。
4. **导完恢复原状 —— 仅当本次要导的页签全部导完后才执行**（中途还有其它页签要导就先别还原，攒到最后一次性恢复）：
   - ① **还原页签顺序**到第 1 步记录的原始顺序；
   - ② **删除**为导出临时创建的**备份表**。
