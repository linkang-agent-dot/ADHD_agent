...
**所有本地化修改的最终结果必须写入「整理版」页签（`复活节-整理版`），而不是原始的「卡册key（复活节）+本地化」页签。**

整理版内有**两个区域**需要同步更新，缺一不可：

| 区域 | 行范围 | 列 | 内容 |
|------|--------|---|------|
| 【卡包本地化】 | 数据从 R95 开始，共 24 行 | Col B = 中文名/描述，Col C = 英文名/描述 | 12个卡包 × name+desc |
| 【卡包描述（修改版）】| 数据从 R122 开始，共 22 行 | Col B = 修改后中文，Col D = 英文翻译 | Col A（原文）不动 |
