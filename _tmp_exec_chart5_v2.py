# -*- coding: utf-8 -*-
"""执行图表5 SQL - R级分层付费情况，2026-03-13~04-02，科技节 %2112%"""
import sys, json
...
select '全体玩家' as label, 
        ...
        sum(COALESCE(pay_total,0)) as pay_total 
from pay_info
union all 
...
print("=== 执行图表5: R级分层付费（科技节 2026-03-13~04-02） ===")
try:
    rows = api.execute_sql(SQL, "TRINO_AWS")
    print(f"查询成功，共 {len(rows)} 行")
