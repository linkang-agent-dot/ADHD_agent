...
GREEN=rgb("8EE085");MOD=rgb("D9EAD3");PEACH=rgb("FCEBCC");PINK=rgb("FF99FF");LBLUE=rgb("CFE2FF")
rows=json.loads(gws(['sheets','spreadsheets','values','get','--params',json.dumps({"spreadsheetId":SID,"range":f"'{TAB}'!A1:L245"})])).get('values',[]); N=len(rows)
...
SEC=("〇、包装层","一、设计目的","二、玩法说明","三、规则说明","配置表/复用源","界面交互","本地化","【美术需求】")
for i,r in enumerate(rows):
    ...
    elif b.startswith("【深海节·视觉调性】"):
        reqs.append(cell(i,1,12,{"backgroundColor":LBLUE,"textFormat":{"bold":True}},"userEnteredFormat(backgroundColor,textFormat)"))
    elif b in SEC or b.startswith("·复用"):
