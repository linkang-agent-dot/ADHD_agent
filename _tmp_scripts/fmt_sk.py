...
GREEN=rgb("8EE085");MOD=rgb("D9EAD3");PEACH=rgb("FCEBCC")
rows=json.loads(gws(['sheets','spreadsheets','values','get','--params',json.dumps({"spreadsheetId":SID,"range":f"'{TAB}'!A1:L200"})])).get('values',[]); N=len(rows)
...
for i,r in enumerate(rows):
    ...
    elif re.match(r"^〇、|^[一二三]、",b) or b in ("配置表（内嵌）","界面交互","本地化") or b.startswith("·复用"):
...
for k in range(0,len(reqs),50):
    ...
    print(f"batch {k}:", 'OK' if 'replies' in o or 'spreadsheetId' in o else o[:100])
print("reqs:",len(reqs))
