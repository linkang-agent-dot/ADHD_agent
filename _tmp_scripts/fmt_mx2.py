...
rows=json.loads(gws(['sheets','spreadsheets','values','get','--params',json.dumps({"spreadsheetId":SID,"range":f"'{TAB}'!A1:H35"})])).get('values',[]); N=len(rows)
...
for col,w in [(0,120),(1,150),(2,420),(3,230),(4,110)]:
...
for i,r in enumerate(rows):
    ...
    if a.startswith("深海节 · 活动美术需求"):
        reqs.append(cell(i,0,8,{"backgroundColor":GREEN,"textFormat":{"fontSize":14,"bold":True}},"userEnteredFormat(backgroundColor,textFormat)"))
    elif a.startswith("【深海节 · 视觉调性】"):
        reqs.append(cell(i,0,8,{"backgroundColor":LBLUE,"textFormat":{"bold":True}},"userEnteredFormat(backgroundColor,textFormat)"))
    elif a in ("主色","风格","表现手法","通用基调","避坑"):
        reqs.append(cell(i,0,1,{"textFormat":{"bold":True}},"userEnteredFormat.textFormat"))
    elif a=="模块":
        reqs.append(cell(i,0,5,{"backgroundColor":HDR,"textFormat":{"bold":True}},"userEnteredFormat(backgroundColor,textFormat)"))
    elif a and a[0] in "①②③④⑤⑥⑦⑧⑨":
        reqs.append(cell(i,0,1,{"textFormat":{"bold":True}},"userEnteredFormat.textFormat"))
    elif a.startswith("★"):
        reqs.append(cell(i,0,8,{"textFormat":{"bold":True}},"userEnteredFormat.textFormat"))
