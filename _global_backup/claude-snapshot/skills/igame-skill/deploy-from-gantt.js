...
const IGAME_QUERY = path.resolve(__dirname, 'scripts', 'igame-query.js');
...
function checkPrerequisites() {
  ...
  if (!fs.existsSync(IGAME_QUERY)) {
    errors.push(`igame-query.js 不存在: ${IGAME_QUERY}`);
  }
...
function checkDuplicates(configId, startTs, endTs) {
...
async function main() {
  ...
  for (const act of toDeploy) {
    ...
    for (let si = 0; si < segments.length; si++) {
      ...
      // ─── Duplicate check ───
      const conflicts = checkDuplicates(act.configId, startTs, endTs);
