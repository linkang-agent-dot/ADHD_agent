...
**⭐ 开单服活动一律优先用组合脚本**（自动 getservertime → timegm 换算 → deployserveractivity，杜绝手算时间戳翻车）：

```bash
python scripts/gm_combo.py open-activity --server 330 --player 14122 --actv 102993 --days 7 --env beta
python scripts/gm_combo.py servertime --server 330 --player 14122 --env beta       # 只查服务器逻辑时间(已实测)
python scripts/gm_combo.py advance-time --server 330 --player 14122 --days 1 --env beta  # 跨天/推进时间(不可回退),完事自动复查
```

> `--player` 给该服**现存**玩家（清过服的旧ID会静默返回 `returnInfo:[]`）；不确定就先 `python scripts/mongo_query.py` 或 pymongo 捞 `ServerPlayer` 前几个 `_id`。
...
**⭐ 一条龙脚本（beta 服部署一律优先用这个，别现场编排）**——自动带同主机 Map/Center、等 20s 复查 Status=Run、Down 自动重试一次，即「部署规则」4 条铁律的固化：

```bash
python scripts/deploy_beta.py --server 220 --tag dev_festival   # Center auto(换分支才带)
python scripts/deploy_beta.py --server 220 --query              # 只查现状
# 退出码 0=全 Run 且 tag 对；2=重试后仍异常需人工
```

仅 beta kadmin（110-510）。dev(inner) 环境和本地服 3080 不适用，走下面的手动方式 / memory `workflow_x3_local_server_gm_telnet`。
