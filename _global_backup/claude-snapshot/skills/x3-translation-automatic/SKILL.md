...
### 陷阱 5：`--reskin-residue` 的误报判读（2026-07-28 马戏节实证，别看到 RESIDUE 就改）

残留检查**只按词命中，不区分 key 的归属**——**源节日自己的 key 必然被报出来**（马戏节那轮 3 条命中全是误报：`TXT_ActvOnline_ActvName/ActvDesc_102246`＝深海节本尊的"航海通行证"，"航海"本来就对）。

**判读三步**：
1. 该 key 属于哪个活动（`TXT_ActvOnline_*_<AO_ID>` 直接读 ID；Pack/RuleTips 类往下走）
2. 该活动的 **ActvGroup（AO col38）是不是目标节日**——不是就是源节日自己的，跳过
3. 共享类 ID（RuleTips / Pack / ScoreID）查 **谁在引用它**：`ActvOnline col13` 找 RuleTips 引用方。只有「**目标节日的活动引用了含源节日名的文本**」才是真残留。
   - 实例：`TXT_RuleTips_Content_16041` 含"深海"被报 → 查引用方只有 `AO 100598 深海累充`(group140)，马戏累充用的是独立的 `16044` → **误报**，且顺带证明 must-check「RuleTips 禁共用」守住了。

⛔ **改错的代价是双向的**：把误报当真残留去改，会**把源节日的正确文案改坏**（那些 key 还在线上服役）。

### 陷阱 6：直写 tsv 建的新 key 不要再跑 CompositeI18n 扫描

若新 key 是**直接写进 `tsv/i18n/Text__Text.tsv`** 的（如批量克隆活动时随配置一起落的自动 key），后续补翻译走**场景 A：直接改语言列**即可。**别为了"保险"再跑一次扫描**——扫描会把这些行重新判定并标回"新增"，已填的译文在下一轮又被当成待翻项重翻，白做一遍且可能漂移术语。
判据同正文「文本修复前的判定」：**CN 源文对不对**是分水岭，对就只动翻译层，不碰扫描。

### 陷阱 7：`i18n_leak_audit.py` 对「裸换行多行 cell」是**假阴性**，泄漏会彻底隐形（2026-07-29 实证，比假阳性更危险）

Text 表里有些单元格含**真换行**（无引号包裹），一个逻辑行跨多个物理行。脚本用 `csv.reader` 逐行读，会在换行处**截断**：

- 实例：`TXT_RuleTips_Content_40002` 的 **th 列**在泰语正文后面粘了 2 行旧版中文残留。
- csv.reader 只读到泰语部分 → `CJK_LEAK = 0`（**判定干净**）；
- 剩下的中文被当成一条 key 为 `2.投入扭蛋币+…` 的"新行"，又被 `'TXT_' in k` 过滤掉 → **整条泄漏彻底不可见**。
- 行数对不上是信号：csv.reader 读出 27,318 行，实际只有 26,572 个逻辑行。

**正确姿势＝逻辑行重组**：只有以 `TXT_` / `Text_` / **`LC_` / `lc_`** 开头的物理行才算新行，其余并入上一行，再按 tab 切列。
⚠️ **`LC_*` / `lc_*` 前缀必须认**（`LC_MENU_guild_redpack_*`、`LC_minigame_coin_pusher_*`、`LC_UNION_ipo_*`）——只认 `TXT_`/`Text_` 的解析器会把它们并进上一行，导致上一行"看起来非空"。

> 📌 修正此前记载：曾把这个坑记成"会误报 EMPTY（假阳性）"。**方向反了**——假阳性只是看着烦，假阴性是真漏。两种都存在：按物理行解析既可能把后续语言列误判成空（假阳性），也可能把尾部泄漏截断隐形（假阴性）。**统一解法都是逻辑行重组。**

### 陷阱 8：按主题词圈范围会漏，实测倍数级（2026-07-29 马戏节实证）

`--grep 马戏` 只报 **14** 条；按 **ActvGroup 反查配置外键**建范围报 **46** 条。漏掉的正是"中文里不含节日名"的那批——巡游地块说明（彩球摊/杂技台）、限时抢购抽奖券、高级扭蛋券礼包、幻紫/鎏金魅影纪念卡。

**正确圈法（两条，按可靠度排序）**：
1. **按配置外键反查**：`ActvOnline__ActvOnline.tsv` 的 col38(GroupId) 取该节日全部活动 → 再顺各表 FK 取到 Item/Pack/ChainPack/MemorialCard/RuleTips 的 ID → 拼出 key 前缀集合。这条最全。
2. **按 git 新增行**：`git diff <基线>..<ref> -- tsv/i18n/Text__Text.tsv` 取新增行的 key。适合"这批新配的有没有漏翻"，但会混进别的案子，需再按 ①分类。
工具：`scan_new_i18n.py`（本 skill scripts 目录，实现了②+逻辑行重组）。
