...
### ★透明化硬规则（生成类透明资产，默认走 grfal remove_background）
> **2026-07-01 规则变更（用户拍板）**：透明抠图**统一默认用 grfal `remove_background`**，不再默认双底差分。做法=**生图时用纯绿幕/纯色实底**（prompt 写 "plain solid green screen background, NOT transparent, NOT checkerboard"）→ 生成后 `grfal remove_background` 抠 → 过透明验证闸门。双底差分**降级为兜底**（仅当 remove_background 在毛发/光晕/半透明软边上吃边/穿洞时才用）。

下列类型产物**必须透明底**，生图后**默认 grfal `remove_background` 抠**（禁用 `generate transparent` 参数——会假透明）：
- **行军表情** march_emoji / dynamic_march_emoji
- **各种 ICON**：skill_icon / activity_icon / 资源图标 / 道具图标 / 小图标 / 英雄碎片图标
- **UI 框/panel/丝带/角标**（uiframe / button 等实底 UI 元素，remove_background 对硬边 UI 最干净）
- **道具相关 DK 资产**（投放进客户端的道具/图标类）

用法：生图用绿幕实底 → `call_grfal.py remove_background --image <绿幕图> --out x.png` → `verify_transparency.py x.png`。
> **兜底·双底差分**（毛发/光晕/半透明软边 remove_background 效果差时才用）：`scripts/transparify_asset.py --prompt "<元素>" --out x.png`（生白底+黑底反推 alpha）；`transparify_dual_bg.py` 是底层。硬要求：两次生图除背景外 prompt 一字不差。
> **例外**：`effect_texture` 走自己的 Alpha 蒙版后处理；`ui_extract*` 是从现成图拆元素；`card_gallery` 带框非透明。

#### ★★透明验证闸门（强制·2026-06-29 新增·worker 落盘前必跑，别跳）
> **背景**：gpt 对 "transparent background" 常输出**假透明**——把灰白棋盘格 / 浅色背景**烤进 RGB 像素**，只四角留 alpha=0。`mode==RGBA` 或 `alpha 范围含 0` 会**误判通过**（实测沙滩椅图标 89% 不透明、棋盘格占 40% 却"看着是 RGBA 透明"）。**所有上面列出的透明类型，落盘前 worker 必须跑确定性验证**：
```
python scripts/verify_transparency.py <产出.png>
```
- 退出码 1 = 假透明/不透明（block：`NO_ALPHA`无alpha / `ALL_OPAQUE`没透明 / `BORDER_OPAQUE`边框不透明=背景没抠 / `CHECKERBOARD`gpt假棋盘格）。
- **不通过 → 走 `grfal remove_background` 抠掉假背景 → 再跑 verify_transparency 复验**（实测 grfal remove 能去棋盘格/浅底·主体恢复不透明·藏宝图/沙滩椅都靠它救回）；仍不过则换 prompt 纯绿幕重出再抠。
- **验证通过（exit 0）才算落盘成功**；worker 在 task.result 里附 `verify_transparency` 统计（透明占比/边框透明/棋盘格占比）。
- 这条覆盖"双底差分/remove_background/gpt直出"任一来源——**最终都要过 verify_transparency 这关**。
...
### ★禁烤文字（2026-07-17 樱花拜访bg实证）
游戏投放类图（bg/陈列图/icon/banner）prompt **一律显式加 "Absolutely NO text, NO letters, NO words anywhere"**——gpt 爱往招牌/黑板/横幅上烤英文（The Blooming Keg 案），烤上=i18n 硬伤只能重出。场景里有招牌/黑板元素时额外写明 "blank sign with emblem only"。
...
17. **写 task json ≠ 派发——两步必须同一动作块完成（2026-07-14 实证）** — 主 agent 写完 `state/tasks/*.json` 后若被对话打断，pending 任务会无限躺尸（本案 3 个任务躺 1 小时被用户问"图呢"才发现）。纪律：①写 json 的同一轮必须紧跟 `Agent(media-worker)` 调用；②收到"图还没到"类询问先 glob tasks 过滤 pending 对账，pending 且无 worker = 漏派，立即补派。
