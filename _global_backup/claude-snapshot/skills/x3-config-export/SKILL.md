...
> 🆕**tsv-schema-gate 机检层（2026-07-16 实证）**：导配置 job 内置独立门禁，**新列插在已有导出列前面=直接 FAILURE**，报错格式 `[tsv-schema-gate] blocker: tsv/<表>.tsv (added columns inserted before existing exported columns)`+钉钉机器人 t2h99bl 报警（张力 ActvBpFund 加列实证）。修法=新列加表尾或占冗余尾列（下述第2条）。看到导配置 FAILURE 先 grep consoleText 里 `schema-gate`。
...
3a. 🆕**CS1061 报 CActv*Cfg 缺字段的判读顺序（2026-07-17 张力集结基金实证）**：先查**配置表头有没有该列**——有=proto 生成类滞后，跑 GenCSharp.py 重生成即修；**没有=代码引用「幽灵列」**（典型成因：queue友被 schema-gate 打回后撤了配置列、但服务端+客户端代码没同步撤，靠旧生成类蒙混编译——任何人重生成 proto 即全组编译炸，双端一起）→找提交人修代码/补列，自己只打**本地不提交**占位补丁绕过。GenCSharp 需 `local.json`（copy local.json.temp 改 client_path=本机 client 路径）。
