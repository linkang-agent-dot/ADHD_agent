...
metadata: 
  ...
  modified: 2026-07-28T13:10:10.286Z
---
...
> 🔴🔴**Note(col4/备注列)完全不可信，查奖励内容一律看 ItemID（2026-07-28 一天误判两次）**：
> 实测世界杯开箱榜 1005 的奖励行，备注写着「金币-推币机BP单服免费Lv9 / 稀有技能书-推币机BP单服免费Lv13 / 材料票…」，
> 而 ItemID 实际是 `5304001`(足球宝贝皮肤) + `11015/11025/11035`(三种加速)——**备注是从别处串行/覆写来的垃圾**。
> 我据此误判「世界杯发的是银币/材料票/技能书这类养成材料」，差点让马戏节跟着改品类；实际两边道具**本来就完全一致**。
> **铁律：对比/核对奖励内容时，只认 ItemID，把 Note 当噪音**；要人类可读的名字，去 `Item__Item.tsv` 按 ID 反查。

## 🪤 玩法返工后必须全库扫旧活动名（2026-07-28 马戏节实证）

活动**玩法/名称变更**（本案 07-10「转盘→开箱」）后，改了活动表还不够——**旧名会散落在下游多张表**，且不报错：

| 残留处 | 性质 | 危害 |
|---|---|---|
| `TXT_Item_Desc_<头衔道具id>` | **玩家可见** | 玩家照说明去找一个不存在的活动 |
| `TXT_ItemObtain_ObtainDesc_<道具id>` | **玩家可见** | 同上（点道具「+」就看到） |
| `PlayerTitle.TXT_获取方式` | 源文 | 同上 |
| `RankRewardSlotCfg` / `Reward` 备注列 | 内部 | 误导后来改配置的人 |

**做法**：改完活动名/玩法后跑一次 `grep -rn "<旧名>\|<旧名英文>" tsv/`（中英都要搜，英文常是 `Circus Wheel` 这类），
把玩家可见的（`TXT_` 开头的 i18n）优先修，英文用**活动的官方译名**（从 `TXT_ActvOnline_ActvName_<AO>` 取，别自己编）。
| 6 | Num | int | ✅ | 数量 |
...
## ★两条会卡导表的硬约束（2026-07-28 马戏纪念卡实战，往现有包加行必踩）
校验代码＝`Tools/table_exporter/def/reward_def.py`。**往任何已有 Reward 包追加行之前先读这两条**，否则本地 ExportTable 直接 Exception。

### ① 同一 RewardID 内 `DisplayOrder`(col14) 不能重复
`reward_def.py:70` — 报错文案 `rewardID:%s DisplayOrder 重复: %s (ID=%s 与 ID=%s)`。
🪤**克隆现有行来加内容时最容易中招**：col14 被一起抄下来 → 同组两行同序。**新增行必须给组内唯一的 DisplayOrder**（取该组现有 max+1）。

### ② 同一 RewardID 内 `ID`(col0) 必须是**连续区间**
`reward_def.py:136-142` — `rewardIds[-1] != rewardIds[0]+len-1` 即报 `rewardID:%s ID不连续, minID:%s maxID:%s`。
🪤**这条最反直觉**：不能像别的表那样"取全表 max+1 追加到末尾"，因为新 ID 跟原包的 ID 段隔了十万八千里。
**正确姿势**：
- 给**现有包加行** → 要么用该包 `maxID+1`（须确认没被别的行占用），要么**整包搬到一段全新的连续 ID 区间**（原行改 col0 + 新行接尾）。col0 只是行唯一号、不被外部引用（外部引用的是 col1 RewardID），**改它是安全的**。
- **新建包** → 整包分配一段连续 ID 即可，天然满足。
- 分配连续段的做法：从一个高位起点扫，找长度 k 的全空闲连续块，边分配边记进 avoid 集防同批互撞。

### ③ 换皮加东西前必查「这个包被几个组共用」
马戏大富翁累计的 8 个包 `4200001-4200008` **同时被组 100(深海)/101(马戏)/102(常驻航海之路) 引用** → 直接往包里加纪念卡＝深海和常驻也跟着发。**必须克隆出专属包再改指向**（本次 → 4200014-4200021，改 `ActvVoyageOtherReward` 组 101 的 8 行 col3）。
反例对照：开箱累计 `30940-30946` 只被组 1016 用＝马戏专属，可直接加行。
**查法**：按引用表的「组列」聚合，看同一个 RewardID 出现在几个组下。
