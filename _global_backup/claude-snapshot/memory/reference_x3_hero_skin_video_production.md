...
- **★另一类=不透明展示视频**（节日/礼包/解锁 showcase，非角色透明）：不走 SBS，就是普通满幅不透明 mp4。**格式锚=`Res/Video/VideoRes/egypt_sphinx.mp4`(尼罗,用户认可的好范本)**：608×1080(9:16)/h264/yuv420p/24fps/~1.5Mbps。同类:summer_love_song(1080×1920)/mecha_unlock(810×1080,3:4)/HeroAdvanture。**分辨率无统一值**(各按展示框9:16或3:4),真标准=「该弹窗/槽位的视频框尺寸」。
...
## 三、演出脚本（角色视角，钩眼为王）
- **★终态架构（必先读，否则会误以为纯静态）**：演出方案最终收敛为 **一条直出片 =「入场表演段 + 落定 + 静态 idle 循环段」整条循环播**，不是纯静态 idle。本章「别动太多」的克制规则**只管后半的 idle 循环段**；前半入场段（拿球跳舞→踩/颠球→兜腋下→落定→表情渐变）是转化钩子，要有戏、不受此约束。入场段编舞 + 单条直出工程铁律见 **第八章**——三章和八章是同一个结论的两半，合起来看。
...
- ★**最终压缩优先 GRFal `compress_video`（大哥反馈效果更好，本地 ffmpeg 手调参数差很多，见 §一顶部）**；⚠️**口语叫 compress_video，GRFal 实际工具名=`video_compression`**（H.265/HEVC,服务端 ffmpeg;参数 crf 默认28越高越小/preset 默认slower/max_height 默认1080/audio_bitrate 默认64k）。落地=`python C:\Users\linkang\.claude\skills\grfal-api\scripts\call_grfal.py --tool video_compression --file video_file=in.mp4 --params "{\"crf\":24,\"preset\":\"slow\"}"`（输入必须走 `--file video_file=` 不是 params）。注意它的 crf≠§四 export_sbs_video 的 quality（那条走 SBS 导出链,别混）。本地视频处理用项目自带 ffmpeg `C:/x3-project/client/Tools/VideoTools/ffmpeg/ffmpeg.exe`(含ffprobe) **仅用于几何操作**(SBS 双半 crop+hstack/裁切/缩放/去音轨)和 compress_video 不可用时应急。cv2 写不了可控码率 H.264(openh264 下到 cwd 能编但码率失控56MB)，要本地编码一律 ffmpeg、别用 cv2。
