...
- **fail-open**：Gemini 任何失败 → 自动回退第一段纯打分，HTML 顶部挂⚠️提示。报告永远出得来。

### ⚠️ Gemini 免费层坑（已踩，已处理）
- `gemini-2.5-pro` 免费层 `limit:0` 根本不可用 → 模型列表只留 `["gemini-2.5-flash","gemini-2.0-flash"]`。
- 免费层有**每分钟**请求/token 限额：反复手测会 429（生产一天1次不会触）；`analyze.py` 有 3 轮重试 + 退避 `[0,25,55]s` 跨过分钟窗口。
...
### 调参入口
recommend.py 顶部 `TASTE_BOOST/COOLDOWN_DAYS`；build_report.py 顶部 `N_ANALYZE(18)/N_SHOW(9)`；analyze.py 顶部 `GEMINI_MODELS/RETRY_ROUNDS/BACKOFF`；口味改 `config\work_profile.md`。
