...
- **GM errCode=6 = no handler（命令名不存在）**，别当权限/参数错。活动查询正名=`GMPrintServerActivityByCfgId <cfgId>`（**没有**裸 GMPrintServerActivity/GMPrintActivity/GMPrintPlayerInfo）；GM 全名单 grep `GMHandler` 于 `ActivityMeta.Gm.cs`。
- **验证客户端改动必让用户停Play→等Unity reimport→重Play重登**：运行中的 Editor 不热加载 bytes/新PNG/prefab；服务器GM开活动成功≠客户端可见（客户端cfg旧→枚举跳过→组图标不出）。

## 给用户发测试道具前先核他实际登录的 uid（2026-07-20 实证）
给 28094 发币，用户实际登 28240，界面显示 0 被误判"配置有问题"。判法=游戏日志统计 uid 频次：`grep -oE "UID\[[0-9]+\]" game-<sid>.<日期>.log | grep -oE "[0-9]{4,}" | sort | uniq -c | sort -rn`，条数最多的是在登的号（GM 自己 @ 的号条数少）。发道具/开活动前先跑这个确认目标号，别默认沿用 memory 里记的旧测试号。
