...
## ⛔ 出图/配图前铁律：先扒上一个节日同槽位图逐项比对（2026-06-23 血泪）
**任何节日活动美术资产（活动背景/礼包小图/HUD图标/门头）换皮或新出图前，必须先调出参考节日（尼罗/夏日/情人节）的同槽位资产，逐项对齐再动手**——别出完/配完进游戏才一张张发现不对（深海拜访礼包就这么把用户耗麻了）。必比清单：
1. **格式**：透明底(RGBA)还是实底(RGB)？礼包小图=**透明抠图+底部缎带横幅**(参 `Egypt_bg_20`)，不是满框实底场景。
2. **构图**：主体在**上半聚焦**、**下半留干净氛围给UI**(参柔情海湾岛屿构图)；下半绝不能堆花草/繁复美术（会和里程碑条/奖励框打架）。
3. **尺寸**：拜访bg标准=**540×960**（尼罗/春节都是；情人节VD_bg_13的1152×2048是异常老规格，别抄）。各槽位尺寸见 [[x3]]「槽位尺寸真相」。
4. **模板元素**：缎带/边框/留白位置照参考摆。
- 实操：`PIL`量参考图尺寸/模式 + `Read`工具直接看参考图长啥样，再写出图prompt（把参考图喂 reference_images）。深海拜访踩坑全集见 [[x3]]「装饰礼包不显示根因」下游 + 本次。
...
- **★装饰礼包要显示必须建活动壳 ActvOnline（易漏·2026-06-23 深海踩坑）**：ChainPack + Pack 配好≠能显示，必须有一条 **ActvType=63（装饰阶梯）的 ActvOnline** 把 ChainPack 挂出来——`ContentID(col5)=ChainPack id` + `ChainPack(col32)=同id` + `ActvIcon(col22)` + `ActvImg(col23)` + `ActvGroup(col39)`。克隆夏日 106101（注意夏日历史 type 写的是 62，正确是 **63**）。深海若 `TimeController=0`（部署驱动）须把 `ACTV_TYPE_DECORATION_LADDER=63` 加进 `PostProcessData.py` 的 `SKIP_TIMECYCLE_CHECK_ACTIVITY_TYPE`，且**别配玩家主体TC（如1830）**——装饰是非玩家活动，详见 [[x3]] "装饰礼包不显示根因"。
