...
**查询踩坑**：
- `2115` 活动任务：**ID 在 col[1]**（col[0] 是 group），查询须加 `--id-col 1`。🛑 **X2 侧纠正（2026-07-08 马戏节基准拉取实测）**：X2 的 2115/2122 读出的行有 p2_title 首列偏移——**ID 实际在 col[2]**（col[1]=group），`--id-col 1` 查 0 行时改 `--id-col 2`。
