...
每轮换皮（尤其多活动批量）开一份档案，作为本轮单一真源——换人 / 换会话读它即可无缝接上。

- **路径**：`C:\ADHD_agent\KB\换皮档案\{项目}\{日期}.md`（文件名即日期）
- **模板**：`C:\ADHD_agent\KB\换皮档案\_模板_换皮档案.md`（复制一份开干）
- **结构**：按活动模块分节，每节记 源活动/母版 + 进度（生成→校验→写入→翻译→美术）+ BUG/解决；底部「本轮速记」记 ID 分配 / 母版规则 / 产出位置 / 阻塞。
- **三步铁律**：① 开工先读 ② 查到 / 决定 / 踩坑随手回写（不留对话里）③ 收工更新进度。
- **分流**：本轮特有信息留档案；「每次换皮都成立」的通用规律回写本文档（§三必检表 / 踩坑记录）。
