...
- ✅ **icon_displaykey(col R) = 节日累充「跑马灯」(顶部滚动通知)左侧图标**（X2-43089 实测确认）：节日累充活动这格常直接填**当期主城皮肤的 city_skin(1312) display_key**（占星=1511020867/拓荒=1511094083，与 1312 表 display_key 同值）。报「跑马灯图标没换/仍是旧紫色城堡」=这格残留上个节日皮肤 DK。校验法：`gsheet_query search 1312 <节日>` 取本节日皮肤 display_key，比对该累充活动 col R。
- 节日换皮常见坑：换皮自带的行(如拓荒节-2026-BP/累充从占星节复制)这两列残留旧节日 DK，要一起换；title_icon(col X,标题栏图)是另一处、和跑马灯/列表图互不替代，按报告对应的 UI 元素分别改
...
## map_emoji 道具图标 DK 约定（2026-06-10 拓荒动态表情 111111326 沉淀）
- **map_emoji 类道具(1111, class=`map_emoji`)的图标 DK = Icon+IconBg 组合，且两条共用同一 key、同一张图、同一 GUID**（占星 1511020886 即 Icon/IconBg 都指 GUID `2c2aed...`）。新建照搬此结构：一张图写两条。
- **DK 图直接指 `Assets/P2/Res/Emoji/Hud/{emoji_id}.png`** 那张表情图本身（占星 1511020886→11800007.png）。emoji_id = 道具 `category_param.effect[].typ=map_emoji` 的 id。
- ⚠️ **节日新表情常是 1024×1024 多帧 sheet(spriteMode 2)**（拓荒 11800011=16帧），**不能整张当 256 图标**（挤成马赛克）。正确：PIL 裁一帧代表帧→256×256 存 `DKUpload/Icon/DK_{key}.png`，meta **直接拷同类单帧表情(11800007.png.meta)只换 GUID**（朴素 Sprite/spriteMode 1/无平台覆盖/2048，非 skill 默认 256-ASTC 套）。裁帧后差分验真透明。
- **节日复制残留**：新表情道具整行从上节日复制，`display_key` 残留旧节日 DK（拓荒 111111326 残留占星 1511020886）→ 要建专属新 DK(全局max+1)再改 display_key；占星旧 DK 不动。改 display_key 是配置表 1111 改动，X2 真源=GSheet（改 tsv 会被冲），走改 GSheet→导表。
- 关联 [[X2 DK 录入正确链路]] 顶部（同文件）+ map_emoji 显示链路见 [[x2-config-library]] §三 X2-43088。
