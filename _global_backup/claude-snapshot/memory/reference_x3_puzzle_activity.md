...
## ★拼图＝纪念卡历史最大的免费出口（2026-07-28 马戏节纪念卡案实证）
- **26 情人节实证**：拼图 5×5 全通 → 纪念卡 **1 张/人**，`1,647 人拿到 1,647 张`；同期兑换商店只兑出 2,428 张/1,624 人。**两条腿合计 4,075 张，拼图独占 40%**，且是唯一对 R0 免费玩家开放的大额出口（`activity_reward` 渠道 R0 参与仅 6-10 人，`activity_exchange` R0 才有 465-1051 人）。
- → **设计节日纪念卡投放时，拼图连线奖/全通奖是"给免费玩家保底几张"的标准位**，别只盯着兑换集市。
- 数据来源：`KB\产出-数值设计\X3_卡册优化\X3纪念卡现状调研.md` 附录A + §7.1；口径见 [[reference_x3_deepsea_memorial_card]]。

## 🪤共用组坑：换皮拼图常与源活动共用奖励组/任务组（2026-07-28 马戏 1829 实证）
- 马戏「小丑的梦境」`AO101829 / CID1829` 的**阶段奖励组＝1100、任务组＝110，与航海之路 CID1828 完全同号**（换皮时直接沿用没克隆）。
- → **要给马戏拼图改奖励（如加纪念卡），必须先克隆出新组再改**，直接改 1100/110 会**连带改掉航海之路 1828**。
- 同类特征还有：`DK_拼图` 仍是 `DK_img_Activity_navigation_bg_9`（航海之路的图）＝美术没换。**判"拼图换皮做完没"＝看奖励组/任务组是不是独立号 + DK 是不是自己的图**，别只看 AO 行和拼图名改没改。

## ★拼图换皮要换的美术是 4 张，不止底图（2026-07-29 马戏节实证：只换了底图，玩家一眼看见格子还是深海贝壳）
`ActvPuzzle__ActvPuzzle.tsv` 有四个 DK 字段，克隆新拼图时**全部会继承克隆源**：

| 列 | 字段名 | 用途 | 规格 | 马戏案继承的错值 |
|---|---|---|---|---|
| col3 | `PuzzleImg` | 5×5 格背后的**底图**（角色场景图） | **804×804** 不透明 | ✅本次已换 |
| **col7** | **`DK_bg1`** | **未点亮格子盖的遮罩图**（深海＝蓝底贝壳） | **160×160** 不透明 | 🔴 `navigation_bg_6` |
| col8 | `DK_bg2` | 背景框 | 84×104 | 🔴 `navigation_bg_7` |
| col9 | `DK_bg4` | 拼图名底图 | 48×68 | 🔴 `navigation_bg_8` |

- **col7 最显眼**——它铺满 25 个格子，不换等于整屏都是上一个节日的元素。判"拼图换皮做完没"先看它。
- 春节 1827 只用了 col3/col7/col8（无 col9），说明 col9 可空。
- 遮罩图（col7）的构造范式：**满幅方块 + 细边框 + 中央单个双色浮雕图案**（浅色图案压在同色系稍深的底上），扁平 UI 风格、无描边、图案约占 70%。换皮＝换配色 + 换中央图案（深海贝壳 → 马戏大顶帐篷）。
