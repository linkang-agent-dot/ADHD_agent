...
metadata: 
  ...
  modified: 2026-07-28T12:16:44.453Z
---
...
🔴**反向失误：「摊清单」不等于「不敢动」——冲突集要算，不能凭感觉（2026-07-28 用户质疑"为啥不能直接拉，很奇怪啊"）**。当时 x3-project 有 52 个在途改动，我看到 16 个 i18n bytes 是 `MM` 状态、判断"pull 会盖掉别人在途工作"，就退回到「只 checkout 需要的 2 个文件」这种打补丁做法，还连着两轮这么干。用户一句话点破。**实际算一遍才发现：52 个本地改动 ∩ 31 个远端改动 = 只有 2 个文件重叠，而且那 2 个恰恰是我自己手工 checkout 造成的**；那 16 个 i18n bytes 早已变干净、根本不在冲突集里。还原掉自己造的 2 个，`git merge` 零冲突、51 个在途改动一个没伤、本地未推送的提交也保住（merge 不是 rebase）。
- **判据（一条命令，别再凭感觉）**：`comm -12 <(git status --porcelain | awk '{print $NF}' | sort -u) <(git diff --name-only HEAD..origin/<分支> | sort -u)` —— 输出为空=可放心 merge；有输出=只需针对这几个文件决定取舍。配合 `git rev-list --count origin/<br>..HEAD` 看本地是否领先（>0 则是 merge 不是 ff，本地提交会被保留）。
- **教训**：在途改动多 ≠ 不能同步。摊清单的目的是**拿数据做决定**，不是拿"可能有风险"当理由退回低效路径；打补丁式地挑文件同步反而会制造出新的不一致（我 checkout 的那 2 个文件后来就成了唯一的冲突源）。同 [[feedback_known_flaky_path_fallback_first]] 的反面：那条讲"别选已知不稳路径"，这条讲"别把稳妥路径想象成不稳就绕开"。
