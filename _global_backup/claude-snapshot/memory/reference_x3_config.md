...
modified: 2026-07-28T07:18:26.428Z
---
## ★让一个活动「不上线/不被触发」的正确做法（2026-07-28 马戏庆功宴实证）

**先破一个误区：把 TC 改成 0 ≠ 活动不会出现。**
`TimeController=0` 只意味着**不绑 TimeCycle、不会自动开启**——但活动仍然：
① 挂在 `ActvOnline.col38 活动组` 里 → **节日 HUD 面板照样显示入口**
② 会被按组批量部署的操作**顺手带上**（TC=0 的活动本来就是靠 iGame 人工部署的，配置层面拦不住部署这个动作）

> ⚠️ 尤其注意：**整个节日排期未定时，全部活动的 TC 本来就都是 0**（马戏节 19 个活动全是）。
> 这时用户说「把 TC 改成 0」多半是误以为 TC 非 0；实际要做的是下面三件。

**三件一起做才真的下线**：
| 处 | 改法 | 作用 |
|---|---|---|
| `col7 TimeController` | 保持/置 `0` | 不自动触发 |
| `col38 活动组` | **清空** | 节日面板不再显示入口；批量部署不会带上 |
| `col1 备注` | 加 `【本期不上线·勿部署】` 前缀 | 给 iGame 部署那步的人看——**这是配置层唯一能拦人工部署的手段** |

**不要删配置行**：内容配置（ActvScore 组/Reward 行等）原样保留，日后恢复只要把活动组填回去，不必重建；真删还容易踩别处引用。

🪤 **下线前必追引用链**（本案实证两处交圈）：
- `ItemObtain` 里有没有 **type=5（跳活动）指向这个 AO** → 有的话道具「+」会跳到一个开不出来的活动
- **规则/说明文案**里有没有提到这个活动名 → 活动不上线，文案就成了错误信息

## X3 BI 打点机制 + UI曝光缺口（2026-06-27 查证）
- **打点框架=服务端** `client\Assets\Scripts\CSSharedHotfix\Game\Server~\BI\BIUserEvent.cs`：`BIUserEvent.Create(player,eventID,eventName,params)`→BI表`user_event`→数仓`v1090.ods_user_event`(列:event_id/event_name/event_paramter/user_id/server_id…)。**客户端UI不直接打点**,要打点须客户端通知服务端fire。
- **全词表≈50事件全是服务端玩法/经济**(升级/建造/create_gift/iap_pop/推币机/竞技场/约会/联盟/拜访/奇观/造船…)。**⚠️零UI曝光打点**:无HUD打开、无活动面板打开、无界面曝光事件→**任何功能的「界面触达率」都无法从数仓测**(没有"打开了哪个界面"的数据),不只竞猜。
- **竞猜零打点**:UIActvWorldCupGuess.cs无上报、全库无竞猜BIUserEvent(连买包都没)。竞猜数仓唯一痕迹=付费订单`ods_user_order.iap_id LIKE '894%'` + 抽奖券`ods_user_asset Item_1146`(混合)。要触达率/漏斗须程序加点(`wc_guess_open`曝光+`wc_guess_pick`选择,或做通用活动曝光点)。
