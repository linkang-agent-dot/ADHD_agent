...
- **分支/存档**：demo 全部提交在 `feature/skin-moment`（含圆选择器/锁定预览/调试入口全套）；三个checkpoint `963e3e0`→`8105a32`→最新(圆选择器+锁定态静态图预览+解锁文案+调试入口,2026-07-13提交)。**demo已提交完整,未合dev(合dev要问用户)**。2026-07-13晚用户让切到 `dev` 分支干别的活(⚠️本地dev落后origin/dev 39提交,干活前先pull)。回skin-moment=`git checkout feature/skin-moment`。视频走LFS，KB备份=`demo\_assets\`
- ⚠️**遗留待验**：场景名Label本地化(圆下"更衣室·耳语/球场·挑逗")之前显示占位"场景名",debug诊断日志已清(提交前),但Label是否绑上/显示对**未最终确认**——回来继续要先跑验,不对则查 SceneCircleTemplate 下 Label 节点名/绑定
...
- **✅圆选择器改自动生成(2026-07-13,用户选B)**：`SkinMomentDemoTool.BuildSceneSelector(root)` 点1-构建Prefab 时自动在 Root 下建 `SceneSelector`(底部居中横排HLG)+`SceneCircleTemplate`(圆模板220px:Frame通用圆框img_cm_bg_icon_touxiangkuang1/Thumb白圆Mask裁圆/SelectFrame白圆染金选中环/Lock锁Common_icon_IconBox2_lock/Label克隆对白Txt继承字体)。用户不用手拼,点构建即生成。编译过。⏳待用户:1构建→Play→菜单2/3实机验证
- **★★正式版方向(用户2026-07-13定)：场景数走皮肤配置表驱动,配几个场景=出几个互动圆按钮**。架构已对——代码 `Scenes[]` 数组驱动、圆按钮按数组长度自动生成。正式版只需把 Scenes[] 从硬编码换成读配置表(HeroSkinMoment 按皮肤ID查场景列表),圆数量/场景名/解锁/视频路径全跟配置走,零手改。**现在先跑demo**
- ⚠️坑:TFWText 继承老版UGUI `Text`(非TMPro)→alignment用TextAnchor/fontSize用int/新建Text无字体不渲染→Label 克隆对白Txt继承字体样式
- **Label「本地化没了」根因+修(2026-07-13)**：Label克隆自对白气泡Txt(深色气泡上的浅奶油字),单独放角色亮背景(白腿)上糊没了+BestFit缩没→修=关resizeTextForBestFit+固定fontSize28+加Outline深色描边(effectColor黑0.9,distance2,-2)。⚠️改动在BuildSceneSelector,防覆盖会保留旧选择器→拿修复要**先删prefab里Root/SceneSelector再点1-构建重生成**
- **「只有一个圆」=正常不是bug**：编辑器只显示1个SceneCircleTemplate(可编辑默认样式),Play时Selector.cs按场景数clone→现2场景出2圆,配置表配N出N。验证=Play→菜单2看2圆
- **★圆里缩略图发黑/没裁圆根因+修(2026-07-13)**：①Mask加错层—Mask裁的是子节点,加在Thumb自身没子节点→运行时preview替换圆sprite等于没裁圆。修=Thumb当圆遮罩容器(白圆Common_WhiteImg_Circle+Mask showMaskGraphic=false)+子节点`Pic`放缩略被父圆裁圆;Selector.cs绑`Thumb/Pic`。②缩略图别用整张竖版全身preview(塞圆里挤成条),用**方形上半身裁图** `SM_104001_{场景}_thumb.png`(脸+胸,gen_thumb.py裁 x居中640/y从5%起640→256²,已落客户端+KB固化)。选择器加载走 ThumbPath 不是 PreviewPath。UGUI圆形头像通则:遮罩容器(圆sprite+Mask)+子图,别把图设成遮罩自身sprite
- **★圆按钮点击区奇怪根因+修(2026-07-13)**：圆模板根(SceneCircleTemplate)只有RectTransform无可点图形,子节点(Thumb/Frame/圆框)全raycastTarget=false→**唯一能点的只剩Label那行字**(克隆对白Txt默认可点)→点圆没反应、点下面小字才切。修=①模板根加透明Image(color alpha0,raycastTarget=true)当覆盖整圆的点击层 ②Label设raycastTarget=false。(alphaHitTest贴圆形命中需sprite可读有运行时报错风险,demo用矩形命中即可)。UGUI通则:EventTrigger监听要生效,该GameObject必须有raycastTarget图形接raycast
- **★Play中重建prefab→运行窗口用旧版看不到新节点(2026-07-13实证)**：现象=日志「圆形场景选择器已生成」+prefab确有SceneSelector节点(unity_prefab_tree验证Root/SceneSelector/SceneCircleTemplate对),但运行时「场景选择器未拼...缺失跳过」=Auto绑定null。根因=**在Play模式里才加的选择器+重建prefab,正在跑的窗口实例是旧prefab加载的**。修=**完全退Play(停止运行不是关窗口)→重新Play重进**,窗口重载新prefab即绑定成功。诊断法=unity_prefab_tree.py验prefab结构对不对(VideoRoot/Dialogue能绑=FindByFullPath机制没问题,同级SceneSelector也必能绑,那就是缓存旧版)
- **假报错诊断(2026-07-13)**：①Console `Assets\Editor\SkinMomentDemo\*.cs error CS`=**旧路径日志残留**(文件早移到GameMainLogicEditor,点Console Clear清掉) ②`AudioManager.Update() NRE(mAudioArr null)`=游戏音频老bug与皮肤无关,根因=没走正常启动流程进Play→从主启动场景登录进主城即消。判编译真成没成看 Editor.log 尾`Loaded All Assemblies`
...
- **★三状态效果图(2026-07-13,真素材PIL合成,拼prefab状态参照)**：`demo\_assets\ui\states\` stateA_lockerroom_play(更衣室可玩默认)/stateB_field_locked(球场未拥有:静态图暗化+解锁文案+解锁按钮+圆带锁)/stateC_field_play(球场已拥有可玩)。全要素:左上返回/右上对白气泡/底部两圆+场景名Label/选中金环/锁/锁定态解锁UI。**合成脚本 compose_states.py**(scratchpad,场景名/位置要调可重跑,用真通用圆框+真锁+真场景图,比gpt渲的准)。⚠️圆里缩略是纯色示意,实际prefab Thumb 填preview裁圆
- ⚠️**又踩两次假结果坑**：components里「锁」之前拷了0个(假结果),真锁=`Common_icon_IconBox2_lock.png`(frame目录,3.5KB)非 common_lock_1；通用圆框确认=`img_cm_bg_icon_touxiangkuang1.png`(CommonNew)。教训重申:文件存在/拷贝成功必看真tool_result
- **后续正式化**：配置表HeroSkinMoment/多语言/StreamedAssets/弹窗队列屏蔽(打开期主城弹窗照弹)/iOS和谐闸门/走MR

### 🕳️ 锁定态静态图预览黑屏——三层根因（2026-07-13 靠日志逐层逮到）
1. **酒馆Bg盖静态图**：视频是AVPro高层渲染(盖住一切)、静态图是普通UI受节点层级管；VideoRoot排在HeroClub酒馆Bg后→视频能显示、静态图被酒馆盖。修=BuildPrefab隐藏 `Root/BackGround`+`Root/Bg`(或手动禁用这俩节点,不用重建)
2. **★VideoRoot被禁用致预览inactive黑屏(硬核)**：锁定态先 `RVideoPlayer.HideEmbedVideo(mGoVideoRoot)` **把整个VideoRoot GameObject禁用**→预览层建在VideoRoot内就跟着 activeInHierarchy=False(SetActive(true)也没用,父节点关着)→黑屏;RoleMask也在VideoRoot内→锁定态点击也失效。**修=预览层挂窗口Root下(`mGoVideoRoot.transform.parent`,永久激活)不放VideoRoot;锁定态点击改由预览层Dim接→ShowUnlockObtain**。挂Root下要SetAsLastSibling后再把mGoSceneSelector提最前(否则盖住圆)
3. **诊断法**：给SwitchScene/ShowPreviewAsync加Debug.Log打 `activeInHierarchy`——一眼看出SetActive(true)但父节点inactive。别猜层级,加日志打真实状态
- 预览层深色兜底:`mPreviewImage.color`先铺深色(0.05)再等图加载,图没到也不露黑视频残留
