...
- **本地服验 dev_festival 配置最快**：`cd Tools/table_exporter && python ExportTable.py`(从 `C:\x3\gdconfig\tsv` 当前分支导出→temp_dev/ProtoGen/*.bytes)→`cp temp_dev/ProtoGen/<改的>.bytes client/Assets/Res/Config/ProtoGen/`(schema没变仅加数据行就行)→重启本地 Game+Map(`--no-build`)或 `!gm ReloadGameServer` 热更。日志验 `StartPreloadData...` 无 `InvalidProtocolBufferException`=配置干净。
