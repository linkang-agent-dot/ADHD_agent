...
## ✅ 验「某 MR 已进 beta 部署」证据链（2026-07-13 储蓄罐 MR!718 实操，无法读 beta 日志/无版本GM 时用这条）
kadmin 看不到镜像 commit（旧坑），闭环走四步：① GitLab API 拿 MR `merge_commit_sha`+merged_at ② Jenkins `x3-server_rebuild` 找合并时间**之后**的 SUCCESS 构建，读 `consoleText` 里 `gitCommitID=<sha>`（构建实际编译的 commit）③ 本地仓 `git fetch` 后 `git merge-base --is-ancestor <merge_commit> <gitCommitID>` 验包含 ④ `deploy_app` 后 query_app 看 **LastDeployTime 刷新为刚刚**（同 tag 重部署会拉 registry 最新镜像，时间不刷=没真部）。注意：registry `dev` 镜像由 game-server-bin CI 异步出，token 看不到该仓（404），只能靠②③间接证；构建 push bin ~构建开始后 10min，镜像再晚几分钟。附：服务端**无模拟购买礼包 GM**（搜遍 GMHandler 充值/购买/支付），验购买写库行为只能客户端真买+Mongo 反查。🪤**在线玩家 Mongo 数据是滞后快照**（定期存盘），刚发生的状态变更查不到→GM `trysaveanddestroy`（BasicMeta.Gm.cs「存盘并尝试销毁」）强制刷盘再查，存盘先行、销毁有状态守卫，在线测试号安全（07-13 验储蓄罐时间戳实测）。
...
## 🔴🔴 配置热更 workflow 会打断 iGame GM 网关注册 → 玩家进不去（2026-07-15 踩坑·亲手救回）
- **配置热更走 kadmin workflow**（不是 deploy_app）：`query_workflow(page,limit)` 必带分页；执行 `execute_workflow(id=)`（内部按 id 查回 name 再发，绕开中文名经 GBK 终端乱码）。**330 = id 1250「beta-330-热更配置+代码」**（两阶段：同步配置+代码 / 热更）；`workflow_history(workflow_id=1250)` 看最新一条 status=success + 两 task executeResult=success 才算成。
- **🔴 同主机有同名「beta-330-清服」(id 1234)**：清 MongoDB 不可逆。执行前**必按 id 查回 name 断言 `'热更' in name and '清服' not in name`** 再发，绝不硬抄 id。
- **🪤 热更是进程内热重载（deploy_app 时间不变，容器没重启）→ 会把服务端↔iGame 内网关的注册弄丢**：热更后 `gm_execute` 全部报 **`serverID:330 error: POST_/ark/gm not support`**，等 2-3 分钟不自愈；**玩家也可能因此登录不进（"游戏进不去"）**。
- **✅ 恢复 = 对该服 `deploy_app(tag='dev')` 整机重部署**（容器重启重新注册网关），~35s Run、再等 **~75s** GM 从 not support 恢复到 Success。**注意重部署后玩家被踢下线**：`opengiftntf` 等在线态 GM 会返回 `Success: []`（空=没在线玩家收到，非成功推送；成功时是 `Success:[{uid,gmResult:"errCode:0"}]`）→ 让用户先登录再重推。
- **口径**：改测试用户配置想生效，别只热更（会踩上面这坑且展示未必变，见下条）；单服测试直接 `deploy_app` 重部署最稳（顺带保证进程重启加载新配置）。
- 🔴🔴**热更 workflow「配置重读」会静默不生效——已实锤(2026-07-16 船只手册D7)**：330 连跑两次 id1250 全 success(两 task 都 success)，服务端实际仍跑旧 ActvOnline/TimeCycle(新玩家身上创建的活动实例窗口=旧 TC 语义)。**workflow success ≠ 配置生效，验证配置类改动必须 deploy_app 重部署**。
- ✅**零副作用「服务端配置新旧」探针 GM**：`removeserveractivitybycfgid <新加的活动cfgId>`(需 playerIds)——返回 `activity config not found`(1017001系)=配置旧；返回 `player X has no activity for cfgId=`(1017012)=**配置已含该活动**=新。不改任何状态(玩家没实例时)。同 GM 也能**下掉玩家个人级活动实例**(自动识别个人/服务器/跨服,`removed N player activities`)，清测试号残留实例用。
- ✅**验「注册相对开启」类活动(TriggerType=3 TC)标准姿势(2026-07-16 船只手册D7全程踩通,四条缺一不可)**：
  1. GM `setplayercreatetime <2026-07-26/00:00:00>` 拨建号时间(getplayercreatetime 先记原值可回拨)。**拨的方向有讲究**：登录补触发守卫(`TimeCycleMeta.cs:478 if(startTime<=lastStartTime) return`)会吞掉"新算开始点 ≤ 已记录上次触发点"的事件——**必须让 新createDate+N天 落在(已记录lastStart, 服务器now]区间内**(即比上次触发记录更晚且窗口已开)。往更早拨(想"更老账号")反而永远不触发。
  2. 先清该号旧活动实例(`removeserveractivitybycfgid`)，否则 BaseActvID 互斥挡新活动创建(兼容设计;还有死缝:TC fire 时旧实例在→挡掉,之后清旧实例也不会补建,本周期错过)。
  3. **活动补建只在玩家实体冷加载时跑(`TimeCycleMeta.OnPostInit`→InitPlayerCreateTimes+InitTimeCycleInfo)**：`setplayercreatetime` 的 OnPlayerCreateTimeChanged 只重算时间**不建活动**。⚠️实体缓存极粘(2026-07-16 实测)：客户端杀进程重开/切服再切回/GM `trysaveanddestroy`(反复 success)**全都没让实体真正冷加载**——**唯一确定性手段=deploy_app 重启该服进程**,重启后随便一次 GM 调用把实体拉起即完成补建(不用等用户登录,GM 拉起→trysaveanddestroy 落盘→Mongo 查 activityDict 即可闭环验证)。
  4. **反推服务器当时跑哪版配置**：看玩家身上活动实例 startTime=建号日零点(D0语义)还是+N天(DN语义)。注册类 TC 的 StartTime 导表时由 timecycle_def.py 转毫秒(`7d 00:00:00`→604800000)、服务端 long.TryParse——机制正确,别怀疑这层。
  - ⚠️`setplayercreatetime` 副作用：所有按注册天数走的内容(新手七日/开局限时礼包/引导期活动)立刻按新号龄处理,新手期内容会消失——**看起来像清号**,对用户自己的号做之前先说明并留原值。
  - 🪤**Unity 编辑器客户端的配置=本地 x3-project 工程 `client/Assets/Res/Config/ProtoGen`**,不跟服务端热更走——服务端建了新活动实例但编辑器客户端配置里没这行(如新增 ActvOnline 行)照样不渲染。验证前让用户 pull 客户端仓(jolt 导表会把新 ProtoGen bytes 提交进 client 仓对应分支)。服务端真相以 Mongo activityDict 为准,展示别当证据。

## 🪤 礼包/奖励类配置验证：展示 vs 到账两条线（2026-07-15）
- **礼包四格「展示」由客户端用客户端自己的配置画**（客户端 build 配置旧→展示还是旧道具，跟服务端热更没关系）；**「到账」（买了发什么）才由服务端配置决定**。→ 验 Reward 改动是否生效，**以到账为准，别信展示**。
- **服务端权威验证 GM**（免真买、免 Mongo 也能先验配置）：`endowreward <RewardID>`（`GMEndowReward(cfgID)`，直接按 Reward 表 ID 投放该奖励到玩家；配 Mongo 反查背包看发的道具 ID 实锤 52003 vs 7002）。
- `opengiftntf <PackID>`（`GMOpenGiftNtf`，仅对 `GiftConst.TriggerPackTypes` 触发型礼包有效）= 给在线玩家推礼包弹窗，方便测展示，不用等自然触发。
...
- 🔴**「不合 dev 单独部某 feature 配置到某 beta 服」走不通(2026-07-16 航海之路 r2 问 330 实证)**：beta 单服的部署+热更都**绑定该服在跑的分支、无分支参数**。① `deploy_app(tag=<feature>)` 无镜像(feature 不建镜像)；② 热更 workflow(如 330=id1250「beta-330-热更配置+代码」)「同步配置+代码」步**拉的是该服绑定分支(如 dev)的 gdconfig，无 branch 参数**(SKILL.md:475-477，task 参数写死绑应用名+路径 `/data/x3/reload/Resource/330/`)→拉 dev 配置不是你的 feature；③ 手动改 workflow 参数/往物理机 reload 目录塞 bytes 要 JumpServer 物理机权限(林康账号无 beta 资产授权)。**即便服务端代码零改(纯配置换皮)、330 跑 dev 镜像已含新逻辑(如 BaseActvID)，配置也进不去**——330 认的配置源就是 dev。**结论：要在 beta 单服验 feature 配置=先合进该服绑定分支(dev)；不想合就本地服 3080(feature 配置直接 cp bytes,纯配置够验)。**
- 🏗️**服务器构建产线换代（2026-07-16 实证）**：`x3-server_rebuild`/`x3-server_rebuild_qamaster` **已禁用**（trigger 报 409 not buildable），新入口=**`x3-serverbuild2`**（参数 branch/publish=Debug，近期 #10 dev SUCCESS）。⚠️当天构建节点标签 `x3-server-build` **无任何在线节点**（x3-dev-cd/x3-online 全离线）→全队列瘫，任何分支都建不了（队列项会一直挂着等节点）。serverbuild2 是否修了 06-29 的 LFS hook 坑未验证（等节点恢复+构建成功+deploy 真拉到镜像才算通）。deploy_beta 试拉不存在 tag 安全：目标服保持原 tag 继续 Run，不会弄坏。
...
- 🔴🔴**本地服代码分支必须跟配置同谱系,否则 cp 新配置崩 UTF-8(2026-07-16 航海之路 r2 血泪 1.5h)**：3080 当时跑 `feature/siren-hud-hide-fix`(落后 dev 66 commit),往它 cp dev 最新配置 bytes 起服→`InvalidProtocolBufferException: String is invalid UTF-8`(`DecoderFallbackException bytes[96]`)崩在 `CfgHelper.StartPreloadData`。根因=代码分支服务端 proto 落后→dev 在其 fork 后改过多张表 schema→新 bytes 按旧 proto 解字段错位→字符串读到二进制。**只 cp 自己改的表也防不住**(落后太多多表 schema 变);**还原成该分支自己的 client ProtoGen 也崩**(环境早脏)。**判据+正解**：本地服验 feature 配置前先 `git -C C:\x3-project rev-list --count origin/dev...HEAD` 看落后多少;落后就先 `git stash push -u`(收 Unity .meta 噪音)→`git checkout dev && git pull --ff-only`→重编 GameServer.Hotfix+MapServer.Hotfix→再 cp 我的表 bytes→起服(dev proto 匹配 dev+我的改动)。别硬 cp 新配置到落后的旧代码分支。
...
## inner dev 集群「全体 Down」诊断链路（2026-07-20 实证）
- **症状**：客户端卡 loading 75%（连不上 Gate 172.20.90.171:10011）+ 本地服 3080 起不来（etcd 172.20.90.151:2379 gRPC DeadlineExceeded）。
- **诊断**：inner kadmin API `POST /api/application-key/query-application-info`（BASE=`https://api-kadmin-inner.tap4fun.com`，Access-Key 见本文件 dev 行；body `{"page":1,"limit":300}` 全量 / `{"name":[...]}` 点查 / `{"fuzzyName":"etcd"}` 模糊）。本次 39 应用 37 Down、仅 mongo/webtool 活 = **宿主机重启容器未自启**（这批应用 restartPolicy=no）；宿主机本身 HeartbeatTime 正常 = 不是机器挂。
- **判「有没有人在修」防撞车**：看 `LastDeployTime`/`LastDeployTag`——有分钟级新鲜的 deploy 记录 = 运维正在操作，**别插手抢 restart**，挂端口监控等恢复。
