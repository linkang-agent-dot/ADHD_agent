...
metadata: 
  ...
  modified: 2026-07-29T10:10:06.550Z
---
...
**🛠 奖池 EV/ROI 通用计算器（2026-07-29 固化，别再现写脚本）**：`scripts\pool_roi.py`——喂池定义(道具/单hit价值/权重)出命中率+EV+EV占比+ROI+免付比，**每次强制打印「最高单项命中率」并按 35%告警/50%红线 标记**。`python pool_roi.py --demo` 跑马戏节扭蛋机样例即模板。

**🪤 EV 守恒 ≠ 体验守恒：删奖池行必须复验命中分布（2026-07-29 马戏节扭蛋机实证）**：为排版把奖池从 7 行砍到 6 行时，把驯养经验整行删掉、权重并进同类的冒险阅历（w100+w100→w200），**期望值分文不差（977.3钻/7.82x）、总权重也没变，但最高单项命中率从 28.4% 翻到 56.8%**——玩家 10 连里平均 5.7 格是同一个道具，直接反馈"抽起来奇怪"。**规律：删行保 EV 的手法（权重匀给同类行）会把命中率堆到单一道具上，EV 校验完全看不出来**。收工必看单项命中率：>35% 警惕，>50% 必被吐槽。修法只有三条且互斥——加回行（EV 分文不动但要改 prefab 排版）/ 接受重复 / 数量翻倍权重减半（命中率降了但总权重下滑会抬高其他行命中率，EV 会失控上涨，本案实测 7.82x→10.9x）。**「行数 / EV / 低重复率」三选二，别指望三全**。
