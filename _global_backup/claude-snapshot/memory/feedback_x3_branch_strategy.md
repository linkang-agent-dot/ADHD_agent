...
## ✅ festival 分支(dev_festival)不走 MR，直接 push / merge（2026-06-30 用户确认）
节日活动批次的改动（gdconfig 配置 + x3-project 代码都一样）在 `dev_festival` 上**直接 commit + push（或合并）即可，不用开 MR review**。`workflow_x3_protected_branch_mr` 的 MR 流程是给受保护主干（dev/master）的；当期 festival 线上的活动开发不受此约束。所以给 festival 分支提 MR = 多余，别问"要不要走 MR"，直接推。
