...
- **★i18n 审计必查两类**(`audit_wc_i18n.py`):缺语种(空) + **中文泄漏**(en/其他列含汉字=没翻,**en列=中文是最易漏判的"伪完整"**);**坑:jp(日语汉字)/zh(繁中)是中文检测的假阳性必须排除**。圈定 = 新增ID段 ∪ 主题词 ∪ **key前缀(如 `_WC_`)**(cn像"胜负预言·32强"无主题词、key又不在ID段,只靠主题词会漏)。
