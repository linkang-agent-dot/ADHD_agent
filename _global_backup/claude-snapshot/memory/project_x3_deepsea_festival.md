...
- **★关键纠正(接管必看)**：①~~拼图101828必须单独iGame部署~~ **⚠️2026-07-30 纠正：这条结论是错的**——代码 `ServerActivityBasicMeta.cs` 的 `CreateGroupActivityIds()` 证明 **ActvGroupSchedule 管激活**（宿主实例启动时自动 `CreateNewServerActivity(子活动, Master.OriginSeaArea, …)`，DurationType=2 ⇒ 与宿主同始终、=1 ⇒ 按 DurationTime、StartTime 是相对宿主的偏移，子活动还继承宿主海域）。深海当时要手动提单，是因为 **GroupSchedule 里那条绑定行(10005)还没配**（行 ID 排在推币机 10717 之后＝后插入）。**正确判据 → 见 [[reference_x3_actvgroupschedule_binding]]**：开工前查 ActvGroupSchedule 有无该子活动行且 IsOn=1，有＝不用提单(窗口自动继承宿主)、没有＝必须提单。**别再凭印象决定，提错会撞出两个实例。**②**拜访105605不是硬7天**(排期约束表软约束·机制=每天领邀请函+阶段里程碑·非酒馆那种阶段时长=活动时长硬校验)→14天不报错·里程碑档位不止7·用户拍板走14天。③AO 105605的ContentID是**5606**不是5605(走ActvVisitPack表·CID在col1)。
