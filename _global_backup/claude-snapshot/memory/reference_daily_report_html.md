...
metadata: 
  ...
  modified: 2026-07-27T13:56:24.396Z
---
...
## ⚠️ 渲染器要扛住"模型换排版"（2026-07-27 修）
日报 txt 由 `claude -p` 每天现写，**排版会漂**：07-27 那版突然改用 `====` 框、`── N. 标题 ────` 小节头、`·` 项目符号 + 80 列硬回车折行。老渲染器只认 `1./①/-/*`，全掉进"普通段落"分支 → 一行一个 `<p>`，页面碎成一堵墙（132 个 `<p>`、0 个列表）。
**结论：日报生成端不可控，渲染器必须容错**。已加进 `render_report_html.py`（备份 `.bak.20260727`）：
- `====` 纯装饰行丢弃；`────`/`━━━`/`═══` 纯行 → `<hr>`；`── 标题 ────` → `<h3>`
- 列表标记扩到 `·` `•` 和状态 emoji（✅❌🔄⚠️⭐🔴🟡🟢🛟🎯）
- **缩进续行并回上一条**（列表项或上一段），中文↔中文拼接不加空格、涉及英数才留空格（`join_wrapped`）
修完今天 132`<p>`→4，49 `<li>`/9 `<h3>`；老格式（07-26/07-15/_daily_plan）逐字节零差异，07-21/22/24 只变好。以后再遇"日报页面很怪"先看是不是又漂了新符号，往这三处加规则，别改生成端 prompt。

**顺带（同日）**：顶栏由 1 个按钮改成 2 个——「📋 复制全文」+「📋 复制一句话版」。起因是一句话版渲染成卡片后拖选复制会碎（序号/标题/描述各成一块），而唯一那个按钮复制的是整份全文。`extract_oneliner()` 从源 txt 抠【一句话版本】小节塞进隐藏 `<pre id="copysrc1">`，没有该小节就不出这个按钮（如 _daily_plan markdown）。教训：**卡片好看但不可复制**，凡是给人"拿去粘贴"的小节，都要配一个复制按钮而不是指望拖选。
