...
- ✅**dev侧退款回收缺口已补(2026-07-07·bf522acc3db随MR!669)**：dev独有方法`ActivityMeta.RecyclePaySignActivityRewardOnRefund`(PayMeta.Refund.cs L163调用,dev_festival没有)原按`Pack==packCfgId`只认基础档——豪华220004退款不回收。修=匹配条件加Pack2。**回收口径自动正确的原因**：回收(L163)先于`RevertGiftPurchaseStateOnRefund`(L194)执行,IsLuxuryPurchased未清,GetReward对豪华买家按×2+满勤大奖展开=与实发一致;二选一硬拦截保证玩家只持一档,无交叉。⚠️此修只在dev(方法本身dev独有);若dev_festival将来合入该方法需带此修。
