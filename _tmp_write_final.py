...
def write_sheet(sheet_name, values, sleep_before=5):
    ...
    params = json.dumps({'spreadsheetId': SPREADSHEET_ID, 'range': f'{sheet_name}!A1', 'valueInputOption': 'USER_ENTERED'}, ensure_ascii=False)
    ...
    if ok:
        try:
            rows = json.loads(out).get('updatedRows', '?')
        except: pass
    status = f'OK rows={rows}' if ok else f'FAIL rc={result.returncode}'
    with open(r'C:\ADHD_agent\_tmp_final_log.txt', 'a', encoding='utf-8') as f:
...
# ====== 5月占星节-甘特 (already done, refresh) ======
write_sheet('5月占星节-甘特', [
    ['X2-5月占星节 甘特图'],
...
write_sheet('6月拓荒节-甘特', [
    ...
    ['联动礼包', '联动礼包-墙纸 [6月新增]', '搬运', 'linkang', '', 'Y','Y','Y','Y','Y','Y','Y', '全程-shop外显'],
    ...
    [],
    ['6月外显线：主城皮肤-GACHA | 装饰物-大富翁 | shop墙纸/墙壁/柜台-联动礼包'],
    [],
    ['Y=活跃  空白=未启动  ?=待确认'],
])
...
write_sheet('7月烟火庆典-甘特', [
    ...
    ['时段', '上半：主城皮肤', '', '', '', 'Y','Y','Y','Y','Y','', '', '', '', '', ''],
    ['时段', '下半：行军特效', '', '', '', '','','','','','Y','Y','Y','Y','Y', ''],
    ...
    [],
    ['亮点：挖孔小游戏唯一S级，专攻大R/超R，仅在下半节日(行军特效期间)启动'],
    [],
    ['R级覆盖：', '小R-团购(上半)', '中R-团购+转盘', '大R-挖孔(下半)', '超R-挖孔+行军特效'],
    [],
    ['Y=活跃  空白=未启动  ?=待确认'],
])
