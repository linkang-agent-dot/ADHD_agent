...
def get_dashboard_detail(dashboard_id: str, include_charts: bool = False) -> dict:
    ...
    chart_ids = list({w.get("chartsId") for w in raw_widgets if w.get("chartsId")})

    # 批量拉 chart detail（走 dashboard-query 路径，只需 Dashboard 权限）
    viz_name_map = {}  # visualizationId → vizName
    chart_name_map = {}  # chartsId → chartName
    chart_detail_map = {}  # chartsId → full detail dict
    if chart_ids:
        chart_detail_map = batch_chart_details(chart_ids, from_dashboard_id=dashboard_id)
        for cid, cd in chart_detail_map.items():
            chart_name_map[cid] = cd.get("name", "")
            for v in (cd.get("visualizations") or []):
                vid = v.get("id", "")
                if vid:
                    viz_name_map[vid] = v.get("name", "")
    ...
    for w in raw_widgets:
        ...
        cid = w.get("chartsId")
        vid = w.get("visualizationId")
        if cid and cid in chart_name_map:
            item["chartName"] = chart_name_map[cid]
        if vid and vid in viz_name_map:
            item["vizName"] = viz_name_map[vid]
        pure_widgets.append(item)
...
def create_dashboard(name: str, description: str = "", tags: list = None, auto_query: bool = False) -> dict:
    body = {"name": name, "description": description, "customTags": tags or [], "aiAction": True}
...
def config_params(dashboard_id: str) -> dict:
    ...
    chart_details = batch_chart_details(unique_chart_ids, from_dashboard_id=dashboard_id)
