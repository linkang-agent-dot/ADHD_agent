# -*- coding: utf-8 -*-
"""
拉长时间查一番赏和推币机的2月周期，看是否从更早开始或更晚结束
"""
import sys, io, json
...
FROM (
    ...
    WHERE partition_date BETWEEN '2026-01-10' AND '2026-03-20'
      AND pay_status = 1
...
for name, days in sorted(by_name.items()):
    ...
    last = days[-1][0]
    print(f"\n{'='*60}")
    print(f"{name}  total=${total:,.0f}  {first} ~ {last}  ({len(days)}天)")
    print(f"{'='*60}")
    for dt, rev, buyers in days:
        bar = '#' * int(rev / 300)
        print(f"  {dt}  ${rev:>8,.0f}  {buyers:>4}  {bar}")
