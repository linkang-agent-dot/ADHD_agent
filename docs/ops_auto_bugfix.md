...
| 下单 BUG 细节 + 截图下载复用脚本 | **`C:\Users\linkang\.claude\scripts\_jira_fetch_issue.py`** — 用法 `PYTHONIOENCODING=utf-8 python _jira_fetch_issue.py X2-43060 X2-43058 ...`（key 作参数，token 仍从 `bug_scan_prompt.txt` 提取不进命令行）。输出 summary/description/全部评论，并把附件下到 `C:/Users/linkang/{KEY}_序号.png`。新增 BUG 完整排查时直接用，不要再现写下载脚本 |
| Bash 终端 Jira 中文乱码（GBK） | 脚本加 `PYTHONIOENCODING=utf-8` 并重定向到文件再 `Read`，不要直接 print（两个 `_jira_*.py` 都已支持写文件） |
