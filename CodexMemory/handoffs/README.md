...
> **Git 边界：**本目录生成的全部运行态（包括用户原话、checkpoint、final、state、index 与 transcript）只保留在本机，禁止进入 Git；仓库只追踪本 README 与 `.gitignore`。
...
- `transcript-snapshot.jsonl`：尽力生成的本机原始记录快照，不作为恢复唯一真源。
