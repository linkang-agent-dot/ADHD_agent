...
- **skill_icon_style_grid.png**：已有技能图标风格参考图（多角色多技能示例），生图时可选作为 reference_images 第三张传入。主流程见 **x3-media** SKILL.md 与 `references/type-skill-icon.md`。
...
py -3 <grfal-api路径>/scripts/call_grfal.py --tool generate_image --params-file params.json \
  --file reference_images=<x3-media路径>/scripts/skill_icon_reference/template_256x256_circle.png \
...
```bash
py -3 <x3-media路径>/scripts/skill_icon_reference/gen_template.py
```
