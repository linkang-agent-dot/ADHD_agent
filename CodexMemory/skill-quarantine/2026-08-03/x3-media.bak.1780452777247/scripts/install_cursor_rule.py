#!/usr/bin/env python3
"""将 x3-media 的 Cursor 规则安装到目标项目的 .cursor/rules/。

初始配置或换项目时使用；可重复执行（覆盖同名文件）。

用法:
    python install_cursor_rule.py
    python install_cursor_rule.py --project <unity_project_root>
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Copy rules/x3-media.mdc → <project>/.cursor/rules/x3-media.mdc"
    )
    parser.add_argument(
        "--project",
        "-p",
