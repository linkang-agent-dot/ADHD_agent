...
X3 项目统一媒体生成 Skill — 所有图片、视频、3D 及媒体处理请求的唯一入口。
...
| 行军表情 (march_emoji) | 游戏内贴纸式单帧表情，透明底 256×256 |
| 动态行军表情 (dynamic_march_emoji) | 固定 4×4 半身循环逐帧表，每格 256×256，透明底 1024×1024 |
...
│   ├── type-march-emoji.md   # 行军表情（单帧）流程
│   ├── type-dynamic-march-emoji.md # 动态行军表情（逐帧）流程
