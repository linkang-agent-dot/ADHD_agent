...
> ⚠️ **首次使用前需配置 X3 项目技能表（Agent 必须主动提示）**
> 下方流程中引用的配置表 `1964_x2_HeroSkillStatsSheet`、表路径 `x2gdconf/fo/json/HeroSkillStatsSheet.json`、Google Sheet 链接均为 X2 项目遗留。**X3 项目对应技能表尚未配置**。
> Agent 在用户首次触发 `skill_icon` 类型时，**必须**停下来向用户索要：
> 1. X3 技能表的本地 JSON 路径（或线上 Google Sheet 链接）
> 2. 表名 / 字段映射（hero_id、skill_id、技能描述字段名）
>
> 用户提供后，Agent 把信息更新到本文件下方的「技能描述来源」段落，再开始生成。
