...
| **质感** | 半厚涂笔触，奇幻元素 + 少量蒸汽机械点缀（齿轮、铆钉、管道作为细节而非主调），色彩饱和鲜明 |
...
- **2D 卡通风格**：`2D illustration, cartoon semi-painted rendering, light steampunk fantasy aesthetic, game item icon style`
...
```
single 2D game item icon, completely transparent background, no frame no border no circle, 2D illustration with cartoon semi-painted rendering, light steampunk fantasy aesthetic with subtle gear and rivet accents, vibrant saturated colors, warm golden and metallic tones, clean crisp edges, subtle soft shadow underneath, game UI asset style, no text no labels no numbers no letters, single centered object
```
