...
- 处理：检查确认残留的 33560→33164→27908 是第一次测试进程链后精确终止；终止后的 `Get-Process` 复查因目标已不存在返回 exit 1，不能误判为终止失败。第二次最小测试同样先按命令行精确定位并清理。hooks 用“是否在模型响应前创建 smoke handoff 目录”验证加载，业务逻辑继续以直接事件模拟和单元测试验收，不把模型调用长时间无输出等同于功能失败。
- 状态：open
