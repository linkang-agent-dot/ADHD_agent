...
MERGE_GROUPS = {
    "大富翁系列": [
        "节日大富翁", "节日大富翁礼包", "2025异族大富翁",
        "异族大富翁随机礼包", "2025复活节大富翁礼包",
        "节日大富翁组队礼包",
    ],
    "弹珠GACHA": [
    ...
    "节日皮肤GACHA": [
        ...
        "一番赏GACHA礼包",
        "2025周年庆-装饰许愿礼包",
        "25万圣节gacha每日礼包", "25感恩节本体gacha每日礼包",
        "2025音乐节GACHA礼包", "2025音乐节随机GACHA礼包",
        "2025音乐节GACHA每日礼包",
    ],
    "行军表情": [
...
for i, (name, fest_data, total, members, module, is_merged) in enumerate(all_items, 1):
    tag = "[合并]" if is_merged else "[单独]"
    revs = "  ".join(f"{fn}:${fest_data.get(fn, 0):,.0f}" for fn in festivals)
    lines.append(f"\n{i:>2}. {tag} {name}  [{module}]  总计 ${total:,.0f}")
    lines.append(f"    {revs}")
    if members:
        lines.append(f"    包含: {', '.join(members)}")

lines.append(f"\n合并后共 {len(all_items)} 行（原 82 行）")
lines.append(f"  其中合并组 {sum(1 for x in all_items if x[5])} 个")
lines.append(f"  未合并 {sum(1 for x in all_items if not x[5])} 个")

result = '\n'.join(lines)
with open(r'C:\ADHD_agent\_tmp_merge_preview.txt', 'w', encoding='utf-8') as f:
    f.write(result)

print(f"Done: {len(all_items)} items after merge (from 82)")
