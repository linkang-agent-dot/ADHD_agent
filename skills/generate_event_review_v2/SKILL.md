...
description: 节日单体数据 Review（Notion + Wiki 双版本）。七维深度分析（触达/行为/付费整体/R级/转化/奖励/礼包）+ 7 张图表 + AI 分析（异常检测/区间划分/活动定位/合理性评估）。支持多对标活动对比。基于 data-analysis + doc-coauthoring 最佳实践优化。
tags: [event, analysis, data, review, holiday, monetization, r-tier, conversion]
...
> 基于 **data-analysis** + **doc-coauthoring** 最佳实践优化
...
### Data Analysis 最佳实践（Stage 1-2）

1. **数据理解优先**: 分析前先理解数据结构与含义，检查字段定义
2. **渐进式分析**: 从简单指标到复杂分析逐步深入
3. **可视化驱动**: 用图表发现模式，用数据验证假设
4. **假设验证**: 所有结论必须基于数据，避免主观臆断
5. **可复现性**: 分析代码与结果完整记录

### Doc Co-Authoring 工作流（Stage 3-5）

本 skill 整合三阶段文档协作流程：

- **Stage 1 (数据收集)**: 收集上下文，明确分析目标
- **Stage 2 (分析与迭代)**: 迭代完善分析，头脑风暴式探索
- **Stage 3 (报告输出)**: 读者测试，确保报告可读性
...
### Stage 1: 数据收集与校验（整合 Data Analysis）

> 借鉴 data-analysis 的数据探索与验证方法
...
### Stage 2: 分析与可视化（整合 Data Analysis）

> 借鉴 data-analysis 的统计分析 + 可视化最佳实践
...
### Stage 3: 报告组装（整合 Doc Co-Authoring）

> 借鉴 doc-coauthoring 的迭代完善流程
...
#### Doc Co-Authoring 迭代完善

在报告生成过程中，遵循 doc-coauthoring 的迭代流程：

1. **头脑风暴式探索**: 对每个分析维度生成 3-5 个可能的角度
2. **假设验证**: 所有结论必须引用具体数据支撑
3. **读者视角检查**: 关键指标是否解释清晰，非专业人员能否理解
...
> 借鉴 verification-before-completion 的质量验证流程
...
## Data Analysis 最佳实践（Constraints）

### 必做规则 (MUST)

1. **原数据保留**: 分析前创建数据副本，不修改原始数据
2. **过程文档化**: 每步分析逻辑记录在案
3. **结论验证**: 所有发现必须基于数据，无数据不结论

### 禁止事项 (MUST NOT)

1. **数据泄露**: 敏感用户信息禁止在报告中暴露
2. **主观臆断**: 禁止无数据支撑的结论
