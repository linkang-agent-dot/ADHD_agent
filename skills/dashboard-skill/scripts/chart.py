...
def create_chart(name: str, sql: str, datasource_type="TRINO", catalog="hive",
    ...
    body = {"name": name, "type": "SQL", "aiAction": True}
    ...
    sql_body = {"id": chart_id, "type": "SQL", "sql": sql, "dataSourceType": ds, "catalog": catalog, "aiAction": True}
...
def create_visualization(chart_id: str, viz_type: str, viz_name="",
    ...
    # 查已有 viz，有则更新，无则创建（viz 是写操作，需要图表编辑权限）
...
def get_chart_detail(chart_id: str, from_dashboard_id="") -> dict:
    """获取图表完整详情。有 from_dashboard_id 时走 dashboard-query 路径（只需 Dashboard 权限）。"""
    if from_dashboard_id:
        ...
        if not data:
            raise RuntimeError(f"图表 {chart_id} 在 Dashboard {from_dashboard_id} 中未找到或无权限")
    else:
...
def _build_dashboard_arguments(dashboard_id: str) -> list:
    ...
    # 遍历图表级 arguments.defaultValues 补齐独有参数（走 dashboard-query 路径，只需 Dashboard 权限）
    chart_ids = list({w.get("chartsId") for w in (raw.get("widgets") or []) if w.get("chartsId")})
    if chart_ids:
        chart_detail_map = batch_chart_details(chart_ids, from_dashboard_id=dashboard_id)
        for cd in chart_detail_map.values():
...
def query_charts_batch(chart_ids, dashboard_id="", arguments=None,
    ...
    if arguments and dashboard_id:
        ...
        for cid in chart_ids:
            cd = chart_detail_map.get(cid)
            if not cd or "arguments" not in cd:
            ...
            for a in chart_args_def:
                kw = a.get("keyword", "")
                if not kw or kw in provided_keys or a.get("defaultValues"):
                    continue
                # 尝试从数据源查询候选值
                source = a.get("source", "")
                source_id = a.get("sourceId", "")
                filled = False
                if source == "FILTER_SQL" and source_id:
                    try:
                        fs_result = api_post(f"/filter-sql/execute/id/{source_id}").get("data", {})
                        rows = fs_result if isinstance(fs_result, list) else fs_result.get("result", [])
                        if rows:
                            vals = [str(r.get("value", r.get("name", ""))) for r in rows if r.get("value") or r.get("name")]
                            if vals:
                                a["defaultValues"] = vals
                                auto_filled.append(f"{kw}=[{len(vals)} values](from filter_sql)")
                                filled = True
                    except Exception:
                        pass
                elif source == "DIMENSION" and source_id:
                    try:
                        dim_data = api_post(f"/dashboard-query/dimension/{source_id}", body={"wheres": []}).get("data", [])
                        if isinstance(dim_data, list) and dim_data:
                            vals = [str(d.get("value", "")) for d in dim_data if d.get("value")]
                            if vals:
                                a["defaultValues"] = vals
                                auto_filled.append(f"{kw}=[{len(vals)} values](from dimension)")
                                filled = True
                    except Exception:
                        pass
                if not filled:
            ...
            if missing:
                skipped[cid] = {"skipped": True,
                                ...
                                **({"autoFilled": auto_filled} if auto_filled else {})}
            else:
...
def main():
    ...
    p.add_argument("--arguments", default=None, help="参数定义 JSON 数组，覆盖自动推断（修正 source/quotation/dataType）")
    ...
    p.add_argument("chart_id")
    p.add_argument("--type", default="SQL", choices=["SQL", "CUBE"], help="图表类型（默认 SQL）")
    ...
    try:
        if args.action == "create":
            ...
            sql = _resolve_sql(args)
            arguments = parse_json_arg(args.arguments) if args.arguments else None
            ...
            result = fork_chart(chart_id=args.chart_id, chart_type=args.type)
