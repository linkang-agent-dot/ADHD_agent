#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""X3 深海节收入日监控 — 全天页签 HTML 日报（2026-07-03 克隆 x3_worldcup_daily.py，仅改配置块+配置读取走 git show origin/dev）
...
DATASOURCE = "TRINO_HF"
FESTIVAL_NAME = "深海节"
# 深海节 = D35+ 老服(开服≤2026-05-30)一次性铺开的双周节日（双周双核心：W1转盘breadth / W2大富翁+BP depth）。
#   主体 14 天 = UTC 2026-07-03 00:00 → 2026-07-17 00:00（D1–D14 = 7/3–7/16），iGame UTC 直填部署（单号13916-13933）。
#   D0 口径：2026-07-03 00:00 UTC = 07-03 08:00 北京 → UTC 日切后 D0=07-03、开场 UTC hour=0。
FESTIVAL_D0 = "2026-07-03"
# ⚠️ 节日活动下线时点（必填）：主体窗口 7/16 24:00 UTC = 北京 07-17 08:00 为上界。
# 必须加这道时间上界：白名单里的许愿池 1002001 是跨节日复用包，下个活动会重新挂上来卖。
# ⚠️ 已知口径重叠：世界杯窗口(6/26–7/20)完全罩住深海窗口，两活动同池服并行；
#   唯一共用包 = 许愿池 1002001（7/3–7/16 销量会同时被算进世界杯与深海两份日报，金额小可接受）。
#   深海 BP 用 130035/036/037/046，与世界杯的 130020/021 不撞。
FESTIVAL_END_TS = "2026-07-17 08:00:00"  # 深海窗口（07-03 08:00 → 07-17 08:00 北京 = UTC 7/3 0点 → 7/17 0点）
FEST_TIME_GUARD = f"o.created_at < TIMESTAMP '{FESTIVAL_END_TS}'"
BASELINE_START = "2026-06-19"       # 基线 14 日（D0 前；⚠️ 6/26 起世界杯已在跑，基线=「世界杯进行中的常态盘」，
BASELINE_END = "2026-07-02"         #   量的是深海在世界杯之上的增量；深海专属包此窗口零流水已由 D0 前无售验证）
# 服段（深海=固定 59 服单段，套世界杯"全服单段"模型，无分批/无滚动）：
# - 主报告(深海) = D35+ 老服 59 个，server_id 1170–2010 非连续（合服空 id 已剔，2026-07-02 数仓拉取定案）。
# - 对比侧(夏日恋语第一批) = 1000–1870（88服，D0=05-29 时已过 D35 的成熟服）；59 vs 88 服，
...
SERVER_FILTER = f"AND TRY_CAST(o.server_id AS INTEGER) IN ({_DEEPSEA_SERVERS})"    # 深海 59 服（显式清单，别用 BETWEEN）
SERVER_FILTER_VAL = "AND TRY_CAST(o.server_id AS INTEGER) BETWEEN 1000 AND 1870"   # 夏日恋语第一批 88 服
SERVER_LABEL = "【UTC 日切】深海节 59 服(server_id 1170–2010 非连续显式清单)；对比侧夏日恋语第一批 1000–1870(88服)，服数不同→绝对值不可比、仅比率可比"
...
RECHARGE_ACTV_ID = "100598"   # 深海节-累充 ContentID（其 RechargePointPackWhitelist = 深海礼包全集 45 包）
ALWAYS_INCLUDE_PACKS = []

# PackType → 模块名（默认兜底，主要服务对比侧夏日包；深海 45 包全部走下面 PACK_OVERRIDE 精确归模块）
PACKTYPE_MODULE = {
    "11": "付费连锁", "15": "抽奖券锚点", "16": "礼包",
    "18": "通行证", "26": "成就礼包", "3": "常规礼包", "7": "每日特惠", "1": "皮肤礼包",
}
# 深海 45 包逐一归模块（2026-07-03 按 origin/dev 白名单 + Pack 名核对；PackType 11/16 各混了多组语义，全量拆开）
PACK_OVERRIDE = {
    # 每日礼包（怦然心动深海版，ActvType29 进度礼包，$4.99×5 档，7/3–7/9）
    "800005": "每日礼包", "800006": "每日礼包", "800007": "每日礼包", "800008": "每日礼包", "800009": "每日礼包",
    # 转盘付费连锁（ChainPack701，「深海礼包」$4.99–$99.99 五档）
    "211022": "转盘连锁", "211024": "转盘连锁", "211026": "转盘连锁", "211028": "转盘连锁", "211030": "转盘连锁",
    # 藏宝图锚点包（「深海藏宝秘卷」$4.99/19.99/49.99/99.99）
    "13021": "藏宝图锚点", "13022": "藏宝图锚点", "13023": "藏宝图锚点", "13024": "藏宝图锚点",
    # 大富翁罗盘连锁（「航海罗盘礼包2-6」）
    "207104": "大富翁罗盘连锁", "207106": "大富翁罗盘连锁", "207108": "大富翁罗盘连锁",
    "207110": "大富翁罗盘连锁", "207112": "大富翁罗盘连锁",
    # 大富翁成就礼包（「深海寻宝礼包」PackType26，11 档）
    ...
    # 通行证（深海双 BP：远航日志+航海通行证，高级 130036/046 + 至尊 130035/037）
    "130035": "通行证", "130036": "通行证", "130037": "通行证", "130046": "通行证",
    # 其余单点模块
    "211016": "装饰礼包", "211017": "装饰礼包", "211018": "装饰礼包",   # 深海居所特惠
    "211019": "头像框礼包",     # 深海头像框礼包 $9.99
    "211020": "拜访礼包",       # 海滨之约 $99.99
    "1002001": "许愿池",        # 跨节日复用包（与世界杯口径重叠的唯一一包）
    "2026101": "节日周卡", "2026102": "节日周卡", "2026103": "节日周卡", "2026104": "节日周卡",  # 7/9 随新包开
    "280001": "存钱罐",         # 深海珍宝罐（大富翁 PiggyBank）
}
# 配置读不到时的兜底快照（= 2026-07-03 origin/dev ActvOnline 100598 白名单全量 45 包）
FALLBACK_PACK_IDS = [
    "800005", "800006", "800007", "800008", "800009",
    "211019", "130046", "130035", "211016", "211017", "211018", "211020",
    "211022", "211024", "211026", "211028", "211030",
    "1002001", "2026101", "2026102", "2026103", "2026104", "130036", "130037",
    "207104", "207106", "207108", "207110", "207112",
    "2801001", "2801002", "2801003", "2801004", "2801005", "2801006",
    "2801007", "2801008", "2801009", "2801010", "2801011",
    "280001", "13021", "13022", "13023", "13024",
]
...
# 基础配色（未列出的模块从调色板自动分配）
BASE_COLORS = {
    "转盘连锁": "#6c63ff", "藏宝图锚点": "#ffd166", "大富翁罗盘连锁": "#34d399",
    "大富翁成就礼包": "#22c55e", "通行证": "#fb923c", "许愿池": "#38bdf8",
    "每日礼包": "#f472b6", "节日周卡": "#c084fc", "存钱罐": "#2dd4bf",
    "装饰礼包": "#fbbf24", "头像框礼包": "#f43f5e", "拜访礼包": "#fb7185",
    "付费连锁": "#60a5fa", "抽奖券锚点": "#facc15", "皮肤礼包": "#ec4899",
...
# 上期节日首日对比基准（夏日恋语第一批 D0=2026-05-29，1-88服 = 深海的直接前代：深海=夏日验证 breadth 后主攻 depth 的迭代）
# 两节日均 北京08:00=UTC00:00 开场，对比按"已跑小时数"对齐
BENCHMARK_NAME = "夏日恋语"
BENCHMARK_D0 = "2026-05-29"
# 夏日节日礼包判定（2109xx 夏日专属装饰/拜访 + 复用的情人节 2107xx 连锁/锚点 + 通用通行证/许愿池；
#  窗口限定在夏日 D0 起，2107xx 不会误吞情人节当期流水）
BENCHMARK_FEST_PRED = "(o.iap_id LIKE '2109%' OR o.iap_id LIKE '2107%' OR o.iap_id IN ('130020','130021','1002001'))"
LAUNCH_HOUR = 0  # 两节日开场时点（UTC 小时）：深海&夏日均 北京08:00=UTC00:00 开场，UTC 日切后落在 0 时
...
def main():
    ...
    if not all_days:
        # 上线前（UTC 今天 < D0）：深海节尚未开场，写个占位 latest.html 并正常退出（不报错，避免每小时任务红灯）
        msg = f"深海节尚未开始：UTC D0={FESTIVAL_D0}，当前 UTC 报告日={report_date}（北京 08:00=UTC 00:00 才进入新 UTC 日）。"
        print("INFO:", msg)
        ph = (f"<!doctype html><meta charset='utf-8'><title>X3{FESTIVAL_NAME}日报</title>"
              ...
              f"<p style='color:#8892a4'>口径：UTC 日切 · 全服 1–98(server_id 1000–1970) · 累充 {RECHARGE_ACTV_ID}。"
