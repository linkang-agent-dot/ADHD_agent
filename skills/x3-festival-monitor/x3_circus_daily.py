#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""X3 马戏节收入日监控 — 全天页签 HTML 日报（2026-07-31 克隆 x3_deepsea_daily.py，仅改配置块，沿用 UTC 日切模型）
...
DATASOURCE = "TRINO_HF"
FESTIVAL_NAME = "马戏节"
# 马戏节 = D35+ 老服 87 个一次性铺开的双周节日（W1 开箱/许愿池/累充 breadth / W2 扭蛋机+寻宝+大富翁 depth）。
#   主体 14 天 = UTC 2026-07-31 00:00 → 2026-08-14 00:00（D1–D14 = 7/31–8/13），iGame 部署单 14397–14415。
#   D0 口径：2026-07-31 00:00 UTC = 07-31 08:00 北京 → UTC 日切后 D0=07-31、开场 UTC hour=0。
FESTIVAL_D0 = "2026-07-31"
# ⚠️ 节日活动下线时点（必填）：主体窗口 8/13 24:00 UTC = 北京 08-14 08:00 为上界。
# 必须加这道时间上界：白名单里的许愿池 1002001 / 周卡 2026101-104 是跨节日复用包，下个活动会重新挂上来卖。
FESTIVAL_END_TS = "2026-08-14 08:00:00"  # 马戏窗口（07-31 08:00 → 08-14 08:00 北京 = UTC 7/31 0点 → 8/14 0点）
FEST_TIME_GUARD = f"o.created_at < TIMESTAMP '{FESTIVAL_END_TS}'"
BASELINE_START = "2026-07-20"       # 基线 11 日（深海 UTC 7/17 关、世界杯 UTC 7/20 0点关 → 7/20 起为双节全关后的纯空窗常态盘；
BASELINE_END = "2026-07-30"         #   窗口内航海之路常驻版 102804 在跑，属常态盘）
# 服段（马戏=固定 87 服 D35+ 单段，沿用深海"全服单段"模型；2026-07-31 用户定 87 服而非全服 118，与深海口径可比）：
# - 主报告(马戏) = D35+ 老服 87 个，server_id 1170–2290 非连续（清单取自 igame-x3-activity-deploy\circus_batch_0731.json）。
# - 对比侧(深海节) = 59 服 1170–2010 非连续（同为 D35+ 口径，深海 D0=07-03 时的 D35+ 名单）；87 vs 59 服，
#   绝对值（流水/人数）不可比，只可比率（付费率/ARPU/ARPPU）。
_CIRCUS_SERVERS = ("1170,1270,1310,1350,1390,1400,1420,1440,1460,1510,1530,1540,1550,1560,1570,1580,"
                   ...
                   "2070,2080,2090,2100,2110,2120,2130,2140,2150,2160,2170,2180,2190,2200,2210,2220,"
...
SERVER_FILTER = f"AND TRY_CAST(o.server_id AS INTEGER) IN ({_CIRCUS_SERVERS})"      # 马戏 87 服（显式清单，别用 BETWEEN）
SERVER_FILTER_VAL = f"AND TRY_CAST(o.server_id AS INTEGER) IN ({_DEEPSEA_SERVERS})"  # 对比侧深海 59 服
SERVER_LABEL = "【UTC 日切】马戏节 87 服 D35+(server_id 1170–2290 非连续显式清单)；对比侧深海节 59 服(1170–2010)，服数不同→绝对值不可比、仅比率可比"
...
# 累充活动 ContentID：马戏节 = 100599(马戏累充) / 100600(寻宝累充)，两白名单相同
# 模块分类 = Pack.PackType（config 原生分类）+ PACK_OVERRIDE 精确拆模块
# ⚠️ 配置读取走 git show origin/dev（主仓工作树可能在 dev_festival/脏，直接读会拿到陈旧白名单——2026-07-03 铁律），
#    git 失败才回退工作树，再失败回退 FALLBACK_PACK_IDS 快照。
GDCONFIG_TSV = r"C:\x3\gdconfig\tsv"
GDCONFIG_GIT = r"C:\x3\gdconfig\.git"
# ⚠️ 马戏节累充是双份：100599 马戏累充(W1) + 100600 寻宝累充(W2)。
#   两行的 RechargePointPackWhitelist 完全相同（61 包，2026-07-31 origin/dev 实测 identical），读一份即可。
RECHARGE_ACTV_ID = "100599"   # 马戏节-马戏累充 ContentID（其 RechargePointPackWhitelist = 马戏礼包全集 61 包）
ALWAYS_INCLUDE_PACKS = []

# PackType → 模块名（默认兜底；马戏 61 包全部走下面 PACK_OVERRIDE 精确归模块）
PACKTYPE_MODULE = {
    "11": "付费连锁", "15": "抽奖券锚点", "16": "礼包",
    "18": "通行证", "26": "成就礼包", "29": "BP基金", "3": "常规礼包", "7": "每日特惠", "1": "皮肤礼包",
}
# 马戏 61 包逐一归模块（2026-07-31 按 origin/dev 白名单 + Pack 备注/ItemObtain/ChainPack 实证核对）；
# 同 dict 里保留深海 45 包的归类给对比侧用（ID 不撞；共用包 1002001/2026101-104 两边模块名一致）。
PACK_OVERRIDE = {
    # ===== 马戏节（本期） =====
    # 每日礼包（马戏每日礼包 第1-5日，ActvType29 进度礼包 102994，$4.99×5 档）
    "800010": "每日礼包", "800011": "每日礼包", "800012": "每日礼包", "800013": "每日礼包", "800014": "每日礼包",
    # 开箱豪礼连锁（ChainPack703「活动豪礼」，26马戏节活动-礼包付费1-5，$4.99–$99.99 五档）
    "211036": "开箱豪礼连锁", "211038": "开箱豪礼连锁", "211040": "开箱豪礼连锁",
    "211042": "开箱豪礼连锁", "211044": "开箱豪礼连锁",
    # 马戏门票锚点（ItemObtain 100511 快捷购买 4 档，发 1209 门票）
    "13025": "门票锚点", "13026": "门票锚点", "13027": "门票锚点", "13028": "门票锚点",
    # 扭蛋券礼包（扭蛋高级券：13029-33 道具获取 5 档 + 13034-38 锚点快捷购买 5 档，发 1212）
    "13029": "扭蛋券礼包", "13030": "扭蛋券礼包", "13031": "扭蛋券礼包", "13032": "扭蛋券礼包", "13033": "扭蛋券礼包",
    "13034": "扭蛋券礼包", "13035": "扭蛋券礼包", "13036": "扭蛋券礼包", "13037": "扭蛋券礼包", "13038": "扭蛋券礼包",
    # 寻宝豪礼（拓荒者之城 103101 的门票阶梯 AO103102，付费 5 档，发 1207 寻宝券）
    "81302": "寻宝豪礼阶梯", "81303": "寻宝豪礼阶梯", "81304": "寻宝豪礼阶梯", "81305": "寻宝豪礼阶梯", "81306": "寻宝豪礼阶梯",
    # 寻宝门票特惠（ChainPack708，$19.99×3 档，发 1207）
    "81320": "寻宝门票特惠", "81321": "寻宝门票特惠", "81322": "寻宝门票特惠",
    # 大富翁成就礼包（马戏大富翁成就礼包 第1-11档 10圈→200圈，PackType26）
    "2801012": "大富翁成就礼包", "2801013": "大富翁成就礼包", "2801014": "大富翁成就礼包",
    ...
    "280002": "存钱罐", "280004": "存钱罐", "280005": "存钱罐",   # 马戏大富翁存钱罐 三档
    # ===== 跨节日复用包（马戏/深海两边同名模块） =====
    "1002001": "许愿池",
    "2026101": "节日周卡", "2026102": "节日周卡", "2026103": "节日周卡", "2026104": "节日周卡",  # 周卡 109101 原位改自深海
    # ===== 深海节（对比侧专用，本期白名单不含这些 ID） =====
    "800005": "每日礼包", "800006": "每日礼包", "800007": "每日礼包", "800008": "每日礼包", "800009": "每日礼包",
    "211022": "转盘连锁", "211024": "转盘连锁", "211026": "转盘连锁", "211028": "转盘连锁", "211030": "转盘连锁",
    "13021": "藏宝图锚点", "13022": "藏宝图锚点", "13023": "藏宝图锚点", "13024": "藏宝图锚点",
    "207104": "大富翁罗盘连锁", "207106": "大富翁罗盘连锁", "207108": "大富翁罗盘连锁",
    "207110": "大富翁罗盘连锁", "207112": "大富翁罗盘连锁",
    "2801001": "大富翁成就礼包", "2801002": "大富翁成就礼包", "2801003": "大富翁成就礼包",
...
# 配置读不到时的兜底快照（= 2026-07-31 origin/dev ActvOnline 100599 白名单全量 61 包，与 100600 完全相同）
FALLBACK_PACK_IDS = [
    "13025", "13026", "13027", "13028", "13029", "13030", "13031", "13032", "13033",
    "13034", "13035", "13036", "13037", "13038",
    "81302", "81303", "81304", "81305", "81306", "81320", "81321", "81322",
    "130047", "130048", "130051", "130052",
    "211032", "211033", "211034", "211036", "211038", "211040", "211042", "211044", "211046",
    "280002", "280004", "280005",
    "800010", "800011", "800012", "800013", "800014",
    "820011", "820012", "1002001",
    "2026101", "2026102", "2026103", "2026104",
    "2801012", "2801013", "2801014", "2801015", "2801016", "2801017",
    "2801018", "2801019", "2801020", "2801021", "2801022",
]
...
# ===== UTC 日切（2026-06-26 用户要求）=====
# 数仓 created_at/partition_date 都是北京时间(=UTC+8)。深海节是 UTC 定义的活动(iGame UTC、UTC 0点开)，
# 所以本报表按 UTC 日切：UTC 时间戳 = 北京 created_at − 8h；UTC 日 = date(UTC时间戳)；UTC 小时 = hour(UTC时间戳)。
# - UD：UTC 日(字符串 'YYYY-MM-DD')，SELECT/GROUP/精确过滤都用它，并 AS partition_date 让 Python 侧逻辑不变。
# - HRU：UTC 小时(0–23)，分时图 x 轴；深海&夏日均 UTC 0 点开场 → LAUNCH_HOUR=0。
# - 剪枝：一个 UTC 日跨两个北京分区(UTC d = 北京 [d 08:00, d+1 08:00))，故 WHERE 仍带北京 partition_date 超集([a, b+1])做分区剪枝，再叠精确 UD 过滤。
UD = "date_format(o.created_at - INTERVAL '8' HOUR, '%Y-%m-%d')"   # UTC 日(字符串)
HRU = "hour(o.created_at - INTERVAL '8' HOUR)"                       # UTC 小时
...
# 上期节日首日对比基准（深海节 D0=2026-07-03，59服 D35+ = 马戏的直接前代，同为 D35+ 口径双周节日）
# 两节日均 北京08:00=UTC00:00 开场，对比按"已跑小时数"对齐
BENCHMARK_NAME = "深海节"
BENCHMARK_D0 = "2026-07-03"
# 深海节日礼包判定（= 深海累充 100598 白名单 45 包显式 IN 清单；深海窗口 7/3–7/16 与马戏窗口 7/31+ 不重叠，
#  共用的 1002001/2026101-104 不会双计）
_DEEPSEA_PACKS = ("'800005','800006','800007','800008','800009','211019','130046','130035','211016','211017',"
                  "'211018','211020','211022','211024','211026','211028','211030','1002001','2026101','2026102',"
                  "'2026103','2026104','130036','130037','207104','207106','207108','207110','207112',"
                  "'2801001','2801002','2801003','2801004','2801005','2801006','2801007','2801008','2801009',"
                  "'2801010','2801011','280001','13021','13022','13023','13024'")
BENCHMARK_FEST_PRED = f"(o.iap_id IN ({_DEEPSEA_PACKS}))"
LAUNCH_HOUR = 0  # 两节日开场时点（UTC 小时）：马戏&深海均 北京08:00=UTC00:00 开场，UTC 日切后落在 0 时
...
def _plus1(dstr):
    """'YYYY-MM-DD' → 次日字符串（用于北京分区超集上界）。"""
    return (datetime.strptime(dstr, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")


def utc_range_where(a, b):
    """UTC 日范围 [a, b] 的 WHERE 片段：北京 partition_date 超集([a, b+1]) 剪枝 + 精确 UTC 日过滤。
    a/b 为 'YYYY-MM-DD'。供 `WHERE o.pay_status=1 AND {utc_range_where(...)}` 用。"""
    return (f"o.partition_date BETWEEN '{a}' AND '{_plus1(b)}' "
            f"AND {UD} BETWEEN '{a}' AND '{b}'")


def utc_day_where(d):
    """单 UTC 日 d 的 WHERE 片段：北京分区 {d, d+1} + 精确 UD=d。"""
    return f"o.partition_date IN ('{d}','{_plus1(d)}') AND {UD} = '{d}'"


def _gd_tsv_rows(relname):
    """读 gdconfig 的 tsv：优先 git show origin/dev（主仓工作树可能在别的分支/带脏改动，直接读会拿陈旧数据），
    git 读不到再回退工作树文件。relname 如 'Pack__Pack.tsv'。"""
    import csv, io, subprocess
    try:
        out = subprocess.run(["git", "--git-dir", GDCONFIG_GIT, "show", f"origin/dev:tsv/{relname}"],
                             capture_output=True, timeout=60)
        if out.returncode == 0 and out.stdout:
            return list(csv.reader(io.StringIO(out.stdout.decode("utf-8", "replace")), delimiter="\t"))
    except Exception:
        pass
    return list(csv.reader(open(os.path.join(GDCONFIG_TSV, relname), encoding="utf-8"), delimiter="\t"))
...
def load_festival_packs():
    ...
    try:
        ...
        prows = _gd_tsv_rows("Pack__Pack.tsv")
        ...
        arows = _gd_tsv_rows("ActvOnline__ActvOnline.tsv")
        whcol = None
        for r in arows[:12]:
...
def load_pack_type_map():
    """读 Pack__Pack.tsv → {pack_id: PackType}，给对比用（含夏日包）。读不到返回空。"""
    try:
        prows = _gd_tsv_rows("Pack__Pack.tsv")
        hdr = next(r for r in prows[:40] if "PackType" in r)
...
def query_window(d0_date, fest_pred, h_start, h_end, pack_type_map, sf=SERVER_FILTER):
    ...
    WHERE o.pay_status=1 AND {utc_day_where(d0_date)}
      AND {HRU} BETWEEN {h_start} AND {h_end}
    {sf}
    """
    agg = query(sql)
    if not agg:
...
def cum_date_pred(start_date, last_date, prev_last_date, last_max_hr, partial):
    ...
    if partial:
        return (f"(({utc_range_where(start_date, prev_last_date)}) "
                f"OR (o.partition_date IN ('{last_date}','{_plus1(last_date)}') "
                f"AND {UD}='{last_date}' AND {HRU} <= {last_max_hr}))")
    return utc_range_where(start_date, last_date)
...
def main():
    ...
    report_date = datetime.now(timezone.utc).strftime("%Y-%m-%d")  # UTC 日切：报告日 = UTC 今天（北京 08:00 后才进入新 UTC 日）
    if len(sys.argv) > 1:
    ...
    WHERE o.pay_status = 1
      AND {utc_range_where(BASELINE_START, report_date)}
    {SERVER_FILTER}
    GROUP BY {UD}
    ORDER BY {UD}
    """
    ...
    sql_modules = f"""
    SELECT {UD} AS partition_date, o.iap_id, sum({REV_EXPR}) as revenue
    FROM v1090.ods_user_order o
    WHERE o.pay_status = 1
      AND {utc_range_where(FESTIVAL_D0, report_date)}
      AND {fest_cond}
    {SERVER_FILTER}
    GROUP BY {UD}, o.iap_id
    """
    ...
    if not all_days:
        # 上线前（UTC 今天 < D0）：节日尚未开场，写个占位 latest.html 并正常退出（不报错，避免每小时任务红灯）
        msg = f"{FESTIVAL_NAME}尚未开始：UTC D0={FESTIVAL_D0}，当前 UTC 报告日={report_date}（北京 08:00=UTC 00:00 才进入新 UTC 日）。"
        print("INFO:", msg)
        ph = (f"<!doctype html><meta charset='utf-8'><title>X3{FESTIVAL_NAME}日报</title>"
              f"<body style='background:#0f1420;color:#e2e8f0;font:16px/1.8 -apple-system,sans-serif;padding:48px'>"
              f"<h1>X3 {FESTIVAL_NAME} 日报</h1><p>{msg}</p>"
              f"<p style='color:#8892a4'>口径：UTC 日切 · 马戏节 87 服 D35+(1170–2290) · 累充 {RECHARGE_ACTV_ID}。"
              f"过 UTC {FESTIVAL_D0} 00:00（北京当日 08:00）后本页自动出 D0 数据。</p>"
              f"<p style='color:#8892a4'>生成于 {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC</p></body>")
        with open(os.path.join(OUTPUT_DIR, f"X3{FESTIVAL_NAME}日报_latest.html"), "w", encoding="utf-8") as f:
            f.write(ph)
        sys.exit(0)
    ...
    if BASELINE_HOURLY_DAYS > 0:
        ...
        bl_lo, bl_hi = bl_dates[-1], bl_dates[0]  # 最早/最晚 UTC 日（bl_dates 是 D0-1, D0-2, … 倒序）
        sql_bl = f"""
        SELECT {UD} AS d, {HRU} AS hr,
          round(sum({REV_EXPR}),0) AS total_rev
        FROM v1090.ods_user_order o
        WHERE o.pay_status = 1
          AND {utc_range_where(bl_lo, bl_hi)}
        {SERVER_FILTER}
        GROUP BY {UD}, {HRU}
        ORDER BY {UD}, hr
        """
    ...
    now_date = datetime.now(timezone.utc).strftime("%Y-%m-%d")  # UTC 今天，判断 report_date 是否"进行中"
    # 进行中当天的已跑小时上限（仅 report_date == 今天 时才算"进行中"，否则整日）
    report_max_hr = 23
    if report_date == now_date:
        mh = query(f"SELECT max({HRU}) mh FROM v1090.ods_user_order o "
                   f"WHERE o.pay_status=1 AND {utc_day_where(report_date)} {SERVER_FILTER}")
    ...
    for day in all_days:
        ...
        if s and v:
            ...
            s_where = f"({utc_day_where(sdate)}) AND {HRU} BETWEEN {h0} AND {h1}"
            v_where = f"({utc_day_where(vdate)}) AND {HRU} BETWEEN {h0} AND {h1}"
            cmp_days.append({
...
<div class="header">
  ...
  <div class="meta-row">
    ...
    <span>生成于 {datetime.now(timezone.utc).strftime("%H:%M")} UTC</span>
  </div>
</div>
<div class="container">
  ...
  <div class="section" id="compareSection" style="display:none">
    ...
    <div id="cmpRSection" style="margin-top:20px;display:none">
      <div class="chart-label" style="font-weight:600;margin-bottom:8px">R级对打 · 节日流水 / 层ARPU（上期{BENCHMARK_NAME} → 本期{FESTIVAL_NAME}，各期按当期 R级档位）</div>
      <div class="table-wrap"><table>
