...
DATASOURCE = "TRINO_HF"
FESTIVAL_NAME = "世界杯"
# 世界杯 = 全服同步上线的节日级长活动（区别于夏日恋语的分服龄滚动上线）。
#   系列 hub（开箱/累充/签到/兑换/BP/纪念卡）2026-06-26 全服上线；竞猜礼包按赛程逐场上（6/28+）。
#   D0 口径（用户确认 2026-06-26）：开服 = 2026-06-26 00:00 UTC = 06-26 08:00 北京 → 数仓首日 partition_date=06-26、开场 hour=8。
FESTIVAL_D0 = "2026-06-26"
# ⚠️ 节日活动下线时点（必填）：世界杯决赛 7/19，竞猜+结算尾单到 7/20 收尾，取 2026-07-20 08:00(北京) 为上界。
# 数仓 created_at/partition_date 均为北京时间（开场日 hour=8 暴涨、下线日 hour=8 起零成交，已实测验证）。
# 必须加这道时间上界：白名单里的复用包(通用通行证 130020·130021 / 许愿池 1002001) 下个活动会重新挂上来卖。
# ⚠️ 已知口径外溢：深海节 2026-07-03 全服上线，若深海也复用 130020/130021/1002001，则 7/3–7/19 这三包销量会被算进世界杯
#   （金额相对竞猜礼包很小，可接受；如需严格剥离，后续按活动壳订单标记拆分）。
FESTIVAL_END_TS = "2026-07-20 08:00:00"  # 世界杯窗口（06-26 08:00 → 07-20 08:00 北京，决赛 7/19 + 收尾）
FEST_TIME_GUARD = f"o.created_at < TIMESTAMP '{FESTIVAL_END_TS}'"
BASELINE_START = "2026-06-12"       # 基线 14 日（D0 前；全服此窗口无世界杯专属流水，世界杯礼包 06-26 才全服上线）
BASELINE_END = "2026-06-25"
# 服段（世界杯=全服活动，无分批/无生命周期匹配，套拓荒节"全服单段"模型）：
# - 主报告(世界杯) = 全服 1–98 服：X3 server 号 N → server_id = 1000 + (N-1)×10，故 server 1=1000 … server 98=1970（用户 2026-06-26 细查确认开到 98 服）。
# - 对比侧(情人节) = 情人节 D0(02-06) 时已存在的全服 1000–1540（55服）；世界杯 98 服 vs 情人节 55 服，
#   绝对值（流水/人数）不可比，只可比率（付费率/ARPU/ARPPU）。
SERVER_FILTER = "AND TRY_CAST(o.server_id AS INTEGER) BETWEEN 1000 AND 1970"      # 世界杯全服 1–98 服
SERVER_FILTER_VAL = "AND TRY_CAST(o.server_id AS INTEGER) BETWEEN 1000 AND 1540"  # 情人节全服(其 D0 时已开 1000–1540)
SERVER_LABEL = "【UTC 日切】世界杯全服 1–98服(server_id 1000–1970)；对比侧情人节全服 1000–1540(55服)，服数不同→绝对值不可比、仅比率可比"
...
RECHARGE_ACTV_ID = "100597"   # 世界杯-累充 ContentID（其 RechargePointPackWhitelist = 世界杯礼包全集）
# 世界杯白名单已含全部竞猜/锚点/复用包，口径完整，无需强制并入
ALWAYS_INCLUDE_PACKS = []

# PackType → 模块名（世界杯白名单实测 PackType：11=A线付费连锁 / 15=B线抽奖券锚点 / 16=竞猜礼包(894xx)+许愿池(1002001) / 18=通用通行证）
# ⚠️ 16 在世界杯白名单里只有两类成员：竞猜礼包 894xxx（大头，144 包）+ 许愿池 1002001。
#    故 16→竞猜礼包，许愿池 1002001 用 PACK_OVERRIDE 单独拎出，二者即覆盖全部 16 类成员。
PACKTYPE_MODULE = {
    "11": "付费连锁", "15": "抽奖券锚点", "16": "竞猜礼包",
    "18": "通行证", "3": "常规礼包", "7": "每日特惠", "1": "皮肤礼包",
}
# 个别 Pack 单独成模块（PackType 相同但语义不同，拆开更可读）
PACK_OVERRIDE = {
    "1002001": "许愿池",   # PackType=16 但语义是许愿池，从竞猜礼包里拎出
}
# 配置读不到时的兜底快照（2026-06-26 ActvOnline 100597 白名单：A线 211002/004/006/008/010 + B线 211012-015
#  + 通行证 130020/130021 + 许愿池 1002001 + 竞猜礼包 894xxx 144 包；此处只列非竞猜锚点，竞猜走 config 实读）
FALLBACK_PACK_IDS = ["211002", "211004", "211006", "211008", "211010",
                     "211012", "211013", "211014", "211015",
...
# 数仓 created_at/partition_date 都是北京时间(=UTC+8)。世界杯是 UTC 定义的活动(iGame UTC、UTC 0点开)，
...
# - HRU：UTC 小时(0–23)，分时图 x 轴；世界杯&情人节均 UTC 0 点开场 → LAUNCH_HOUR=0。
...
# 基础配色（未列出的模块从调色板自动分配）
BASE_COLORS = {
    "竞猜礼包": "#6c63ff", "抽奖券锚点": "#ffd166", "付费连锁": "#34d399",
    "通行证": "#fb923c", "许愿池": "#38bdf8",
    "皮肤礼包": "#f43f5e", "常规礼包": "#00d4aa", "每日特惠": "#f472b6", "其他": "#8892a4",
}
...
LAUNCH_HOUR = 0  # 两节日开场时点（UTC 小时）：世界杯&情人节均 北京08:00=UTC00:00 开场，UTC 日切后落在 0 时
...
def main():
    ...
    if not all_days:
        # 上线前（UTC 今天 < D0）：世界杯尚未开场，写个占位 latest.html 并正常退出（不报错，避免每小时任务红灯）
        msg = f"世界杯尚未开始：UTC D0={FESTIVAL_D0}，当前 UTC 报告日={report_date}（北京 08:00=UTC 00:00 才进入新 UTC 日）。"
        print("INFO:", msg)
