...
SERIES = [
    ...
    ("X3深海·转盘族(D0-D6)",    "#ff6b6b", 1, "X3"),
    ("X3深海·大富翁族(D0-D6)",  "#ff9f7a", 0, "X3"),
    ("X3深海·双通行证(D0-D6)",  "#f472b6", 0, "X3"),
    ("X3深海·许愿池单列(D0-D6)","#fbbf24", 0, "X3"),
    ("X3夏日·开箱族(D0-D6)",    "#e879f9", 1, "X3"),
    ("X3深海·开箱(连锁+锚点)(D0-D6)", "#f43f5e", 0, "X3"),
]
...
①<b>图A：$50 以下两边形状几乎一样</b>（X3 甚至 $10-50 段占比更高）——普通买家行为相同；差别在末档「&gt;$100」：P2 各武器有 <b>8~26%</b> 的买家溢出到鲸鱼段，X3 只有 0~13%（转盘族 4.4%、深海开箱 3.8%）；
②<b>图B：P2 的面积从 $100 一路铺到 $2k+</b>（拓荒挖孔 $2k+ 还有 33 人、猜酒杯每个 $100 档都有人住）；<b>X3 的面积在 $500-600 处戛然而止</b>（转盘族 $590 / 大富翁族 $707 / 双通行证 $60 / 夏日开箱 $495 / 深海开箱 $485 = 全货架买光——两代节日的开箱/抽奖族撞在同一堵 ~$490 的墙上，换了节日墙高不变），<b>最肯花钱的玩家被产品拒收</b>；
③P2 人均 $115~$170 vs X3 $29~$42：人均差 ~80% 来自图B 段的缺人——这就是"逐档连锁+限购5"在档位分布上的形状；
④X3 双通行证全部压在图A（$60 买满即止）、许愿池（唯一可复购）能爬进图B 前两档——<b>天花板是 X3 全系统性问题</b>：所有武器按"买全一轮"设计，没有为 top1% 准备无限深度；
<br><b>直接结论</b>：X3 只要给头部 1%（每周约 15 个鲸鱼）一个 $1,000+ 的可花空间（顶档复购 30 次起/随机高档无限购），按 P2 猜酒杯图B 的形状回填 $200-2k 段，仅鲸鱼段就 ≈ $15-20k/周。⚠️ 口径注：X3 均为 D0-D6 七天（深海系=7/3-7/9·59服；夏日开箱=5/29-6/4·批1 88服=情人节复用连锁210702-710双号+券锚点210712-15；深海开箱=转盘连锁+藏宝图锚点、不含许愿池）vs P2 全活动 22-29 天，X3 因窗口短被低估，但图B 断崖与窗口无关（第一天就能买光）。</div>
<script>
const ALBL = __ALBL__;
const BLBL = __BLBL__;
const DIST = __DATA__;
