# -*- coding: utf-8 -*-
"""永恒边锋评价·定稿版页面（骨架=gen_st_final.py，只换权重表+官方位置集+花式列）
公式(用户逐档裁定,2026-07-27):
  终分 = [Σ(属性档位分×档位权重) + 最优特训增益] × 逆足系数
  档位分: 显示值>=165→4(07-28上调) / >=155→2 / >=145→1 / <145→0
  权重: 双速x5 | 盘带·控球·灵活·平衡·弧线·传中x3 | 射术·射门力量·远射·反应·长传·强壮x2
        | 站位·短传·视野·体力·冷静·抢断x1 | 头球·弹跳·凌空·拦截·侵略性x0.5 | 其余x0
  特训: 全卡+5点/单项<=2点, 背包求最大档位增益
  逆足: 5逆x1.0 / 4逆x0.95 / 3逆x0.75（07-27按位置差异化）
  花式: 公式外参考列（边锋吃花式，但池内正牌全4-5星无区分度）
  口径: 接口裸值 +3显示 +8卡强化15 +球员等级4(假设待确证) +队套6 = +28
全员(68人非门将)都算边锋分, >=80进榜+正牌边锋不足80也列出(灰行警示)。
"""
...
NAME_CN = {'Cristiano Ronaldo': 'C罗', 'L. Messi': '梅西', 'T. Henry': '亨利', 'Eusébio': '尤西比奥', 'Ronaldo': '大罗',
 ...
 'O. Solskjaer': '索尔斯克亚'}
COMMENT_WG = {
 '马特乌斯': '中场客串边路，仅作全能数值参考',
 '皮尔洛': '中场客串边路，传控强但双速不足',
 '索尔斯克亚': '中锋客串边路，缺少传中与组织能力',
        ...
        for comb in itertools.combinations(items, r):
            if sum(i[0] for i in comb) <= 5:
                ...
                if g > best[0]:
                    best = (g, list(comb))
    return best
PEN = {5: 1.0, 4: 0.95, 3: 0.75, 2: 0.65}
rows = []
detail = {}
for p in el_list:
    n = p['name']
...
for i, (n, pos, w5, base, g, fin, pct, h, wt, bt, plan, sal) in enumerate(rows, 1):
    ...
    trs += ('<tr class="r%s%s" onclick="pop(\'%s\')"><td>%d</td><td class="nm">%s<span class="en">%s</span></td><td>%s</td><td>%d逆</td><td>%d</td>'
            '<td class="sc">%.1f</td><td>%.0f%%</td><td>%.1f<span class="tg">+%.1f</span></td>'
            '<td>%d</td><td %s>%+.1f</td>'
            '<td class="ph">%dcm/%dkg·%s</td><td class="pl">%s</td><td class="cm">%s</td></tr>'
            % (cross, low, cn, i, cn, n, pos, w5, flair.get(n, 0), fin, pct, base, g, sal, rcls, res, h, wt, bt, plan or '—', COMMENT.get(cn, '')))
...
'<h1>永恒边锋评价 · 定稿版（2026-07-27，用户逐档裁定权重）</h1>'
'<div class="note"><b>终分 = [ Σ(属性档位分 × 档位权重) + 最优特训增益 ] × 逆足系数</b><br>'
'档位分：显示值≥165→<b>4</b>（07-28上调） / ≥155→2 / ≥145→1 / ＜145→0（阈值机制：突破档位才算数，档内堆点无效）<br>'
'权重：<b>双速×5</b>｜<b>盘带·控球·灵活·平衡·弧线·传中×3</b>｜<b>射术·射门力量·远射·反应·长传·强壮×2</b>｜'
'<b>站位·短传·视野·体力·冷静·抢断×1</b>｜<b>头球·弹跳·凌空·拦截·侵略性×0.5</b>｜其余×0（Σ权重48.5，满分194）<br>'
'特训：全卡共+5点、单项≤2点，按背包求最大档位增益　｜　花式：公式外参考列（池内正牌边锋全4-5星）<br>'
'逆足：5逆×1.0 / 4逆×0.95 / 3逆×0.75（07-27调，边锋4逆最宽）　｜　口径：接口裸值+3显示+8卡强化15+<b>球员等级4(假设待游戏内确证)</b>+队套6<br>'
'得分率=终分÷满分194（⚠与中锋页满分132不同量纲，<b>跨位置比较用名次</b>，见全位置拉通页）。'
'<b>全员68人(非门将)都算边锋分，≥80进榜</b>；蓝名=非边锋客串（仅数值参考，无准入门槛）；'
'灰行=不足80的正牌边锋（明确别当边锋买）。身高/体重/模型仅参考不计分。<br>'
'<b>工资残差</b>=终分 − 市场定价线（8名正牌边锋拟合 终分≈' + '%.0f+%.1f×工资' % (INTC, SLOPE) + '，样本小仅参考）——'
'<span style="color:#5cff8f">绿≥+7超模</span> / <span style="color:#ff7b7b">红≤−7工资坑</span>。</div>'
'<table><tr><th>#</th><th>球员</th><th>官方位置</th><th>逆足</th><th>花式</th><th>终分</th><th>得分率</th><th>原分+特训</th><th>薪</th><th>工资残差</th><th>身材/模型</th><th>最优特训方案</th><th>评语</th></tr>'
+ trs + '</table>'
...
'<div class="note">口径：两边同用边锋权重+特训+逆足；档位与主榜一致（145/155/165=1/2/3，'
...
'function pop(n){var d=D[n];if(!d)return;var h="<span class=x onclick=hide()>&times;</span><h3>"+n+" · 边锋分算分清单</h3>"\n'
...
'</script></body></html>')
P = r'C:\ADHD_agent\FCOL阿三资料库\07_数据分析\永恒边锋评价_定稿版_20260727.html'
tmp = P + '.tmp'
