# -*- coding: utf-8 -*-
"""永恒中场评价·定稿版页面（骨架=gen_cam_final.py，换权重表+官方位置集）
公式(用户逐档裁定,2026-07-27):
  终分 = [Σ(属性档位分×档位权重) + 最优特训增益] × 逆足系数
  档位分: 显示值>=165→4(07-28上调) / >=155→2 / >=145→1 / <145→0
  权重(引入x4档): 远射·射门力量·短传·控球·平衡x4 | 双速·盘带·灵活·人盯人·抢断·拦截·强壮x3
        | 视野·体力·反应·弧线·侵略性·射术x2 | 长传·冷静·站位x1 | 传中·头球·弹跳·凌空·铲断x0.5 | 其余x0
  特训: 全卡+5点/单项<=2点, 背包求最大档位增益
  逆足: 5逆x1.0 / 4逆x0.95 / 3逆x0.85（07-27再调回宽松档）
  口径: 接口裸值 +3显示 +8卡强化15 +球员等级4(假设待确证) +队套6 = +28
全员(68人非门将)都算中场分, >=80进榜+正牌中场不足80也列出(灰行警示)。CDM后腰客串=近亲参考。
"""
...
B = 28
OFFPOS = ('CM',)
TIERS = {4: ['longshots', 'shotpower', 'shortpassing', 'ballcontrol', 'balance'],
...
NAME_CN = {'Cristiano Ronaldo': 'C罗', 'L. Messi': '梅西', 'T. Henry': '亨利', 'Eusébio': '尤西比奥', 'Ronaldo': '大罗',
 ...
 'O. Solskjaer': '索尔斯克亚'}
COMMENT_CM = {
 '马特乌斯': '正牌第1：165.5分新中场王；换代+82、工资残差+17.6，双维超模第一候选',
 '皮尔洛': '正牌并列第5：特训+18的卡线怪；换代+80但工资残差仅+1.3，属换代超模而非工资超模',
 '索尔斯克亚': '中锋客串，仅数值参考',
 '图雷': '正牌第2：远射×4与防守项×3双吃；唯一坑=−中场指挥官特性手感笨（公式外），买前自查',
 '卡洛斯': '客串第2：射门力量/控球/平衡×4全是他的菜，4逆×0.90已扣',
 '杰拉德': '正牌第4：远射流双雄之一，×4档全兑现',
 '梅西': '客串',
 '马塞洛': '边卫客串：手感+体力型',
 '齐达内': '客串：手感组通吃',
 '亨利': '客串，特训+14',
 ...
 '大罗': '客串',
 '伊布': '客串：射门力量控球高',
 '哈维': '正牌第3：传控轴心，4逆×0.95已扣',
 '维埃拉': '后腰客串：防守项×3受益',
 '克罗斯': '正牌第8：远射/短传/控球×4受益，5逆不扣',
 '普斯卡什': '客串，4逆已扣',
 '小猪': '正牌第7：特训潜力把他拉回线上——买他=买练卡潜力',
 '坎通纳': '客串',
 '图拉姆': '中卫客串：速度对抗双全',
 '卡卡': '客串',
 '埃托奥': '客串',
 ...
 '伊涅斯塔': '正牌第9：手感组高但×4档（远射射门力量）是短板——纯组织型在这版权重里吃亏',
 '巴蒂': '客串',
 '萨内蒂': '边卫客串',
 '埃辛': '后腰客串',
 '穆勒': '客串',
 ...
 '耶罗': '中卫客串：3逆重扣',
 '托雷斯': '客串',
 '法布雷加斯': '正牌第10垫底：全池中场最弱，档位全面不足+4逆——避雷',
 '欧文': '避雷',
 '菲尔米诺': '避雷',
 '吉鲁': '避雷',
}
...
'<h1>永恒中场评价 · 定稿版（2026-07-27，用户逐档裁定权重）</h1>'
'<div class="note"><b>终分 = [ Σ(属性档位分 × 档位权重) + 最优特训增益 ] × 逆足系数</b><br>'
'档位分：显示值≥165→<b>4</b>（07-28上调） / ≥155→2 / ≥145→1 / ＜145→0（阈值机制：突破档位才算数，档内堆点无效）<br>'
'权重（<b>引入×4档</b>）：<b>远射·射门力量·短传·控球·平衡×4</b>｜<b>速度·加速·盘带·灵活·人盯人·抢断·拦截·强壮×3</b>｜'
'<b>视野·体力·反应·弧线·侵略性·射术×2</b>｜<b>长传·冷静·站位×1</b>｜<b>传中·头球·弹跳·凌空·铲断×0.5</b>｜其余×0（Σ权重61.5，满分246）<br>'
'特训：全卡共+5点、单项≤2点，按背包求最大档位增益<br>'
'逆足：5逆×1.0 / 4逆×0.95 / 3逆×0.85（07-27再调：中场回宽松档）　｜　口径：接口裸值+3显示+8卡强化15+<b>球员等级4(假设待游戏内确证)</b>+队套6<br>'
'得分率=终分÷满分246（⚠与其他位置页不同量纲，<b>跨位置比较用名次</b>，见全位置拉通页）。'
'<b>全员68人(非门将)都算中场分，≥80进榜</b>；蓝名=非中场客串（CDM后腰客串=近亲参考，其余仅数值参考）；'
'灰行=不足80的正牌中场（明确别当中场买）。身高/体重/模型仅参考不计分。<br>'
'<b>工资残差</b>=终分 − 市场定价线（8名正牌中场拟合，样本小仅参考 终分≈' + '%.0f+%.1f×工资' % (INTC, SLOPE) + '）——'
...
'<div class="note">口径：两边同用中场权重+特训+逆足；档位与主榜一致（145/155/165=1/2/3，'
...
'function pop(n){var d=D[n];if(!d)return;var h="<span class=x onclick=hide()>&times;</span><h3>"+n+" · 中场分算分清单</h3>"\n'
...
'</script></body></html>')
P = r'C:\ADHD_agent\FCOL阿三资料库\07_数据分析\永恒中场评价_定稿版_20260727.html'
tmp = P + '.tmp'
