...
| `tm_only_attrs.json` | **时刻独有53人全量**（贝利/马拉多纳/克鲁伊夫等，07-27，53抓零失败）：34项细项+完整db(含foot_weak/salary)；抓取器 `fetch_tm_only.py`（这次连r['db']整存，之前只存子集是坑） |
...
| `gen_wg_final.py` | **边锋模型定稿页生成器（07-27）**：gen_st_final 骨架换权重表/官方位置集/花式列 → `永恒边锋评价_定稿版_20260727.html`；过其余位置继续复制此模式 |
| `gen_cam_final.py` | **前腰模型定稿页生成器（07-27）**：无一档制（最高×3）→ `永恒前腰评价_定稿版_20260727.html` |
| `gen_cm_final.py` | **中场模型定稿页生成器（07-27）**：×4档口径（远射·射门力量·短传·控球·平衡）→ `永恒中场评价_定稿版_20260727.html` |
| `gen_crosspos.py` | **全位置拉通页生成器（可扩展）**：MODELS 字典每加一个位置权重表→自动多两列(分+名次)；跨位置比较用名次口径(得分率因权重表属性数不同不可直接比) → `永恒全位置拉通_得分与名次_20260727.html` |
