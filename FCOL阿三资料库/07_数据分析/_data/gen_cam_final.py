# -*- coding: utf-8 -*-
"""永恒前腰评价·定稿版页面（骨架=gen_wg_final.py，换权重表+官方位置集，去花式列）
公式(用户逐档裁定,2026-07-27):
  终分 = [Σ(属性档位分×档位权重) + 最优特训增益] × 逆足系数
  档位分: 显示值>=165→4(07-28上调) / >=155→2 / >=145→1 / <145→0
  权重(无一档,前腰最高x3): 短传·视野·控球·盘带·远射·灵活·弧线·射门力量x3 | 射术·平衡·反应·强壮·双速x2
        | 站位·冷静·体力·凌空·人盯人·抢断·拦截x1 | 头球·弹跳·侵略性·传中·长传x0.5 | 其余x0
  特训: 全卡+5点/单项<=2点, 背包求最大档位增益
  逆足: 5逆x1.0 / 4逆x0.95 / 3逆x0.75（07-27按位置差异化,前腰0.95推定）
  口径: 接口裸值 +3显示 +8卡强化15 +球员等级4(假设待确证) +队套6 = +28
全员(68人非门将)都算前腰分, >=80进榜+正牌前腰不足80也列出(灰行警示)。
"""
...
B = 28
OFFPOS = ('CAM',)
TIERS = {3: ['shortpassing', 'vision', 'ballcontrol', 'dribbling', 'longshots', 'agility', 'curve', 'shotpower'],
...
NAME_CN = {'Cristiano Ronaldo': 'C罗', 'L. Messi': '梅西', 'T. Henry': '亨利', 'Eusébio': '尤西比奥', 'Ronaldo': '大罗',
 ...
 'O. Solskjaer': '索尔斯克亚'}
COMMENT_CAM = {
 '马特乌斯': '中场近亲客串前腰：双五+远射出球兼备，主场仍是CM/CDM',
 '皮尔洛': '中场近亲客串前腰：传控与远射满档，双速限制推进上限',
 '索尔斯克亚': '中锋客串前腰，组织属性不足',
...
'<h1>永恒前腰评价 · 定稿版（2026-07-27，用户逐档裁定权重）</h1>'
'<div class="note"><b>终分 = [ Σ(属性档位分 × 档位权重) + 最优特训增益 ] × 逆足系数</b><br>'
'档位分：显示值≥165→<b>4</b>（07-28上调） / ≥155→2 / ≥145→1 / ＜145→0（阈值机制：突破档位才算数，档内堆点无效）<br>'
'权重（<b>无一档，前腰最高×3</b>）：<b>短传·视野·控球·盘带·远射·灵活·弧线·射门力量×3</b>｜<b>射术·平衡·反应·强壮·速度·加速×2</b>｜'
'<b>站位·冷静·体力·凌空·人盯人·抢断·拦截×1</b>｜<b>头球·弹跳·侵略性·传中·长传×0.5</b>｜其余×0（Σ权重45.5，满分182）<br>'
'特训：全卡共+5点、单项≤2点，按背包求最大档位增益<br>'
'逆足：5逆×1.0 / 4逆×0.95 / 3逆×0.75（07-27按位置差异化，前腰按技术位0.95推定）　｜　口径：接口裸值+3显示+8卡强化15+<b>球员等级4(假设待游戏内确证)</b>+队套6<br>'
'得分率=终分÷满分182（⚠与其他位置页不同量纲，<b>跨位置比较用名次</b>，见全位置拉通页）。'
'<b>全员68人(非门将)都算前腰分，≥80进榜</b>；蓝名=非前腰客串（仅数值参考，无准入门槛）；'
'灰行=不足80的正牌前腰（明确别当前腰买）。身高/体重/模型仅参考不计分。<br>'
'<b>工资残差</b>=终分 − 市场定价线（5名正牌前腰拟合，样本极小仅参考 终分≈' + '%.0f+%.1f×工资' % (INTC, SLOPE) + '）——'
...
'<div class="note">口径：两边同用前腰权重+特训+逆足；档位与主榜一致（145/155/165=1/2/3，'
...
'function pop(n){var d=D[n];if(!d)return;var h="<span class=x onclick=hide()>&times;</span><h3>"+n+" · 前腰分算分清单</h3>"\n'
...
'</script></body></html>')
P = r'C:\ADHD_agent\FCOL阿三资料库\07_数据分析\永恒前腰评价_定稿版_20260727.html'
tmp = P + '.tmp'
